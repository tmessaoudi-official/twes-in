...
| **zxing-wasm** (the camera's barcode decoder) | `web/package.json`, pinned exactly (`3.1.3`) | `web/src/third-party/zxing-reader/COMPONENTS.json`: `version` and `integrity` must equal the lock's (**gate**) | Read the new tarball first: the zxing-cpp commit it builds (`ZXING_CPP_COMMIT`) and what that compiles in, as `var/claude/decoder-licence.md` did. Then `npm install --save-exact zxing-wasm@<v>`, update the manifest's version, integrity, `audit` line and components, and `php scripts/notices/generate-third-party-notices.php` | The licence gate refuses a bump the manifest has not caught up with, on purpose: a new wasm is unread code. Import `zxing-wasm/reader` only, never the bare package (its default is the full build with the writer) |
