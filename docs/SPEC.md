...
- [2026-09-13] DECIDED (revisit): the notification centre is persisted. Publishing writes one `notification` row per recipient (a `company:` channel fans out to that company's members) and then pushes over Centrifugo; the row is the truth and the push is best-effort, so an unreachable Centrifugo is logged and never fails the use case that published. Centrifugo is reached by the browser only through nginx on the same origin (`/connection/websocket`), and its HTTP API only from the api container.
