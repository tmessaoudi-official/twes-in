...
| R4C-5 | correctness | The `count()` guard is the only thing detecting a save proposing FEWER child rows than are stored, and nothing in the tree proposes one — `childRowsAgree()` iterates `$incoming`, so a shorter incoming set compares only its own rows and returns `true`. The `?? null` covers the opposite direction only | P2 | OPEN |
