...
document lifecycle, numbering and the `Invoice` aggregate, all under `api/src/Domain/Document/`. **Wave 1's persistence, migrations and schema gate all LANDED on 2026-08-01, and the three this sentence then listed as remaining — the HTTP surface, the repository and the mapper — have ALL landed too**: the repository and mapper on 2026-08-06, the READ path with them, and the WRITE path on 2026-08-07. **What Wave 1 still owes is a DIFFERENT list**: `Client`, contacts and `Product`, none of which is an invoice-core deliverable. (The tenant settings table was on that list until 2026-08-21 and is struck from it here rather than left standing — the very defect the rest of this sentence is about.) This sentence named the wrong three for two commits after the invoice core closed out, which is the same shape as the sentence it itself corrects, one wave later. This sentence said they were "blocked on Composer" until round 21 found it — two commits after the passage below was corrected, which is the correction-appended-below-a-false-statement shape `CLAUDE.md` § Gotchas records four times, committed by me twice more while fixing other instances of it. Amended at round 13, which found this sentence and the one in Wave 0's remainder section still saying Wave 1 had not started — in a file the landing commit itself edited. Read
...
- [2026-08-23 10:30] AGREED: **a LINE discount REDUCES the VAT base.** Worked: qty 10 x 15.000 TND = 150.000 gross,
  ...
  models it this way, BT-136 (line allowance) reducing BT-131 (line net amount), and BT-131 is what feeds the VAT
  category base BT-116; the alternative remits tax on money never invoiced. Destination: Wave 2, with the fixture in
  `docs/spec/pricing-vectors.json`.
- [2026-08-23 10:30] AGREED: **a DOCUMENT-level discount is allocated PRO-RATA BY BASE across rate groups.** Worked:
  line1 100.000 @ 19 %, line2 100.000 @ 7 %, document discount 30.000 -> 15.000 off each, VAT 16.150 + 5.950 =
  **22.100**, grand total **192.100**. The rejected reading consumes the discount against the highest rate first
  (VAT 20.300, total 190.300 -- 1.800 TND less VAT). Reason: it REUSES the allocation mechanism already ruled for
  per-line VAT under `PerRateGroup` (largest remainder, ties to the earliest line) rather than introducing a second
  allocation rule, so the codebase carries ONE. The sum-exactly invariant binds here as it does there.
- [2026-08-23 10:30] AGREED: **INCLUSIVE tax extracts PER RATE GROUP, reusing `VatRoundingPoint` rather than adding a
  parameter.** Verified divergence: lines 22.531 + 113.678 at 19 % give per-line VAT 21.747 and per-rate-group VAT
  21.748 -- one millime, and the axis is the SAME one `VatRoundingPoint` already parameterises for exclusive tax,
...
- [2026-08-23 10:30] AGREED: **the runtime role's table privileges come from an IDEMPOTENT GRANT STEP shipping with
  ...
  of the runtime role's NAME -- deployment configuration, which is why this needed a ruling rather than a fix -- while
  making a freshly migrated environment serve a request with no manual step. Rejected: a migration parameter (a
  migration that knows a deployment's role names stops being replayable without it), and provisioning-owns-it-forever
  (least code, most operator burden). Destination: Wave 12, and the gate assertion may land earlier.
- [2026-08-23 09:00] AGREED: **Wave 1's certification cap is extended from 5 rounds to 6, for the sole purpose of
  CONFIRMING a clean round.** MAXIMAL requires two consecutive fully-clean rounds; rounds 1-4 are spent (25, 24, 29
  and 25 findings, none clean), so under the cap of 5 no outcome of round 5 could satisfy the tier -- a clean round 5
  would still be one round short, and the arithmetic was decidable before the round rather than after it. The
  extension is NOT a licence to keep going: round 6 may only confirm, so any finding in round 5 spends the extension
  without using it, and a finding in round 6 resets the counter and escalates. Deliberately NOT the
  bundle-integration precedent, which this plan forbids citing for a code wave -- that work stopped one clean round
  short by ruling; this permits the round that would make the ruling unnecessary.
- [2026-08-23 09:00] AGREED: **all 21 of round 4's open findings close (or are explicitly ruled deferred) BEFORE
  round 5 is spawned**, not just the three P1s. Any single open row resets the clean counter, so freezing with known
  findings would re-file them and burn the extension above on a round whose outcome is already known.
...
- [2026-08-22 18:00] AGREED: **`BehaviouralIsolationTest`'s row synthesiser may return `null`.** `product` is the first table whose CHECK requires a column to be ABSENT rather than merely different, and a synthesiser typed `string` could not express that — it was not a value the map was missing but a SHAPE it could not represent. `scalarFor()` now returns `?string` and `literal()` emits the bare keyword `NULL`. That is a capability gain rather than bookkeeping: any future table with a mutually-exclusive column pair can now be attacked at all, and the evidence is that the suite went from 44 assertions to 128 once `product` could be seeded.
- [2026-08-22 15:00] AGREED: **the client surface ships `POST` and `GET` only, and each missing verb is argued rather than deferred by default.** No `PUT`, because a full replacement must first answer whether a caller may supply contact IDS — the one thing create refuses — and inventing that rule as a side effect of adding an edit endpoint is how a contract acquires a shape nobody chose. No `DELETE`, because removing a client an issued invoice names is a question about a legal document, which is Wave 2's. No collection `GET`, because a list needs pagination decided once, and `CLAUDE.md` § "The API contract is ours to design" rejects upstream's `per_page=999999` outright. The absences are recorded on `ClientResource` where a reader meets them, not only here.
- [2026-08-22 15:00] AGREED: **contact ids are minted server-side, so the aggregate's duplicate-contact and contact-not-found refusals are unreachable from the wire — and they STAY.** A guard deleted because today's only caller cannot trip it is deleted for the wrong reason: a future importer or a `PUT` reaches both. The consequence for § "Translation keys" is stated in that table rather than left to inference — neither gets a key yet, because a key nothing can reach is the declared-but-unconsulted shape this project already records four times, and each earns one in the change that makes it reachable.
- [2026-08-22 15:00] AGREED: **the fifteen `client.*` keys land WITH the surface even though nothing resolves them.** They pay nothing down — `locale-key-parity.php` only checks that the three locales carry the same SET, so deleting all fifteen would leave it green — but a surface shipping fifteen new user-fixable refusals with no keys would make the rule at the top of § "Translation keys" something the next author reads and does not follow. The debt is the typed-exception-per-refusal deliverable, which now covers `Domain/Client/` as well as `Domain/Document/`; adding the keys makes its SIZE visible instead of hiding it. One key per address part and per contact field rather than one carrying a `{what}`, because the interpolated noun is gendered and pluralised differently in French and Arabic — the same reasoning that made `document.line_count_too_large` two keys.
- [2026-08-22 11:00] AGREED: **Client (+ contacts) lands in THREE commits, and is recorded as PART-LANDED between them rather than struck whole.** Domain, then schema, then persistence; the `/api/clients` surface and the `document` → `client` link follow. Recording it as part-landed is the point: this file's own retraction of 2026-08-07 exists because *"still owed"* annotations were struck as things landed while the `In:` line they were a view of went unread, and striking "Client" the moment its domain compiled would be that defect with a shorter fuse. The link is deferred deliberately and not forgotten — it touches `Invoice`, `InvoiceMapper`, the input DTOs, both processors and every existing fixture, and a nullable FK with no backfill is its own decision — so it is named in three places (this log, § Wave 1's heading, `CLAUDE.md`) rather than left implicit in the absence of a column.
- [2026-08-22 11:00] AGREED: **a contact is an ENTITY with its own id; a document line stays POSITIONAL.** The two are deliberately different and the reason is referenceability: nothing ever refers to a line, so `withoutLine(int $position)` is right there, whereas Wave 4 e-mails a PDF to a contact and Wave 10 invites one to the portal. Under positional identity, deleting the first of three contacts silently re-points every stored reference to the remaining two — for *"who do we e-mail this invoice to"* that is a defect nobody sees until the wrong person receives an invoice. The row therefore carries BOTH `id` (identity, and the PK's third column, which is the schema-level statement of the domain's uniqueness invariant) and `position` (order, which is what a user arranged).
- [2026-08-22 11:00] AGREED: **the client field set is DERIVED from EN 16931 — BG-7, BG-8, BG-9 — and the country code is validated by SHAPE only.** Licensing invariant 2 names the standards as a sanctioned input where upstream's code is not, which is what makes this a derivation rather than an invention. The country-code asymmetry with `Currency` is argued rather than overlooked: a currency code is load-bearing for ARITHMETIC (it carries the minor-unit scale, and TND has three), so an unknown one silently produces wrong money; a country code computes nothing and is transcribed, so a well-formed-but-unassigned `XX` produces a document a human can see is wrong. Validating against the real ISO 3166 list belongs to the wave that already validates against Peppol's, where the list is published and checked anyway. **`PostalAddress` is ALL-OR-NOTHING** — a client with no address is the ordinary state and is modelled by `null`, but a city with no country is an address nobody can post to — and required-here is explicitly NOT required-to-issue, which is the e-invoicing wave's stricter check.
- [2026-08-21 14:30] AGREED: **the tenant settings table LANDED, and `vatRoundingPoint` is no longer a field on `CreateInvoice` at all.** The 2026-08-07 ruling above says a client may not choose how much tax a document declares — but that held only at the HTTP boundary, because the *command* still carried the field and any programmatic caller (a CLI import, a Messenger consumer, Wave 6's recurring scheduler) was free to state it. `CreateInvoiceHandler` now reads it from `CompanySettingsRepository` inside its own transaction, which closes the gap for every caller at once instead of one boundary at a time. That 2026-08-07 entry's closing clause — *"every document gets `PerRateGroup` until the settings table exists, which makes `PerLine` unreachable over HTTP today"* — is hereby SUPERSEDED: a company configured for `PerLine` now gets it. `company_settings` is `PRIMARY KEY (company_id)` with no surrogate id, deliberately: it holds exactly one row per tenant and the key is what enforces that, so the composite-key rule is SATISFIED rather than waived — the tenant column IS the key.
- [2026-08-21 14:30] AGREED: **the settings READ requires an active transaction, and that is the control rather than ceremony.** An unbound session and a tenant with no settings row both produce zero rows, and answering the second with documented defaults while the first is a tenancy failure would convert a fail-closed refusal into a silently wrong answer — the exact shape § Gotchas 2026-08-07 records when `bind()` had no call site. The binding is transaction-local by construction, so requiring a transaction makes it STRUCTURALLY PRESENT instead of something each implementation must remember to verify. Defaults are still answered for a missing row, because nothing creates one until Wave 7 brings tenant provisioning, and they reproduce the two retired hardwires (`services.yaml`'s `$numberWidth: 7`, the processor's `PerRateGroup` literal) exactly.
- [2026-08-21 14:30] AGREED: **`BehaviouralIsolationTest`'s GOAL 7 now CHECKS what it previously ASSERTED.** Its finding message claimed *"some uniqueness mechanism on this relation does not include `company_id`"* on the strength of a 23505 alone; that inference held only while every tenant-owned table had a surrogate key beside the tenant column, and `company_settings` refuted it by having the tenant column as its whole key — the probe collided with the ATTACKING tenant's own primary key and reported a cross-tenant oracle against a schema that has none. The key columns are now read from `pg_index` at discovery time (bounded by `indnkeyatts`, so an `INCLUDE` payload is not counted) and the finding is suppressed only on POSITIVE confirmation that the firing mechanism includes the tenant column. **Rejected: skipping such relations outright**, which would also skip a genuine tenant-omitting index sitting beside the tenant-only key. **Rejected: giving `company_settings` a surrogate id to appease the probe** — that reintroduces two-rows-per-tenant to fix a test. `23P01` stays an unconditional finding, because an EXCLUDE constraint chooses its operator per column and `EXCLUDE (company_id WITH <>, …)` includes the tenant column and collides precisely ACROSS tenants; this schema has none today. The suppression is pinned BY NAME to `company_settings`, so a key-map bug that silenced every relation turns that assertion red instead of leaving the findings list quietly empty.
...
- [2026-08-07 04:10] AGREED: **a client may not choose `vatRoundingPoint`.** `PerRateGroup` and `PerLine` produce numerically different tax figures, so a per-request choice would be a client choosing how much tax a document declares. It is company configuration, persisted per document so a later settings change cannot restate a document already sent; every document gets `PerRateGroup` until the settings table exists, which makes `PerLine` unreachable over HTTP today. **(That closing clause is SUPERSEDED — see 2026-08-21 14:30 above. The settings table landed and a company configured for `PerLine` now gets it; the ruling's substance — that a CLIENT may not choose — is unchanged and in fact strengthened, since the field left `CreateInvoice` entirely.)**
...
- [2026-08-06 03:00] AGREED: **a gate must enumerate the SURFACE, not the spellings — and not the locations
  ...
  closing a class rather than an instance. It was, for values; round 28 then walked past it four ways using
  LOCATIONS the gate did not enumerate — `export` in a tracked `.env`, the Dockerfile `ENV`, two of the Caddyfile's
  four seam variables, and a Caddyfile `import`. `scripts/gates/worker-mode-blocked.sh` derives its file set from
  `git ls-files` and its knob set from the Caddyfile's own enumeration, so both axes are closed by construction.
  Recorded as a decision because it is now the shape every new gate in this repository should take: **if a check
  contains a hand-written list of the places or the words it looks for, ask what completes that list, and if the
  answer is "nobody knows", derive it.**
- [2026-08-06 03:00] AGREED: **a security check that needs a daemon belongs in a gate that does not.**
  `compose-config.sh` SKIPS without `docker compose`, and for two commits it was the sole mechanism behind a
  tenancy ruling — with the skip message citing a CI setup this repository does not have. The rendered check stays
  there because only a renderer sees composition; everything checkable as text moved to a gate that cannot skip.
...
- [2026-08-07 01:30] AGREED: **money crosses the HTTP boundary as a DECIMAL STRING, never a JSON number**, and the same for quantities and rates. JSON has one number type and it is a double, so `0.100 TND` — Tunisia's stamp duty, exactly 100 millimes — stops being exact the moment it becomes a float, and three clients parsing it would each round differently. This is money-is-never-a-float enforced at the boundary rather than only in the domain. Asserted on the ENCODED PAYLOAD, not on the PHP type: the property declarations are the enforcement, so `assertIsString()` on them is provably true and PHPStan refuses it — the half-hollow-assertion class this project's configuration already catches elsewhere.
- [2026-08-07 01:30] AGREED: **the server computes totals and the clients display them.** All three clients could compute from the lines; they must not. The calculation involves a rounding point, a rounding mode and a largest-remainder allocation with ties to the earliest line — three independent implementations of that is three chances to disagree about a legal document's total, and upstream maintaining four parallel tax classes kept in step by hand is a large part of why its suite is 167k lines. `docs/spec/pricing-vectors.json` exists so a client that DOES compute (to preview a total) agrees exactly. The rounding MODE is `HalfUp` as a constant rather than a parameter, because the vectors state *"half_up on every amount and every rate. There is NO per-case override"* — a configurable mode would be a way to serve numbers that disagree with the contract every client validates against. The rounding POINT, by contrast, comes from the DOCUMENT, because it is persisted per document precisely so a setting change cannot restate a document a client holds.
- [2026-08-07 01:30] AGREED: **the read resource declares no `Delete`, `Put`, `Patch`, `Post` or `GetCollection`, and each omission has its own reason.** No delete, because an issued document is `cancel()`ed and a delete endpoint would put a way to destroy a legal document beside the way to void one — the same argument that keeps `delete()` off the repository port. No put/patch, because an issued document is immutable and the refusal belongs in the route table rather than in an error response. No collection, because listing needs pagination, filtering and a sort order, which belong to a read model rather than an aggregate repository. `POST` is owed and is its own deliverable.
- [2026-08-07 01:30] RECORDED: **an ill-formed id answers 404, not 400, deliberately.** Distinguishing "malformed" from "absent" tells an unauthenticated prober that its guess had the right SHAPE, which is a small existence oracle for free. Both answers are "no such document" — and so is a document belonging to another tenant, indistinguishably, which is the design of row-level security rather than a limitation of it. Three mutants pin the read path: per-line VAT bypassing the allocation → 1 failure, an absent document not 404-ing → 1, a hardcoded rounding point → 1.

- [2026-08-07 00:30] AGREED: **request-time tenancy is a PORT with a development-only adapter that a GATE refuses in production**, not a header the application quietly trusts. Wave 1's HTTP surface had to be exercisable before Wave 7 writes authentication, and the alternative — no request tenancy until auth — would have left the repository, the isolation strategy and the boundary rule unexercised over HTTP. So `TenantResolver` is the seam Wave 7 swaps, `RequestTenantBinder` binds on `kernel.request` at priority 8 (after the router, before the controller — and deliberately not first, because in Wave 7 that would mean binding before the firewall has decided anything), and `HeaderTenantResolver` is a test double promoted to a service and treated with that suspicion. **Three independent controls, because one is not enough for a defect whose blast radius is every client's invoices**: the resolver refuses LOUDLY when not permitted (a quiet null presents as "you are nobody", which reads as an empty account rather than a rejected request), `TWES_TRUST_TENANT_HEADER` is `0` in `api/.env` AND `infra/.env`, and `scripts/gates/no-forgeable-tenancy-in-production.sh` refuses any other value in a tracked file. Deleted in Wave 7 as a set.
- [2026-08-07 00:30] AGREED: **a tenant-less request CLEARS the context rather than leaving it alone**, and this is the most important line in the seam. `InMemoryTenantContext` is a service — one instance for the container's life — so under a Messenger worker today, or FrankenPHP worker mode when Wave 10 unblocks it, a tenant bound by an earlier unit of work is still there. Leaving it would mean a tenant-less request INHERITING the previous request's tenant: a cross-tenant read arriving through the most innocent path available, an unauthenticated endpoint served correctly. Pinned by two cases and a mutant (inheriting → 2 failures). Sub-requests deliberately do NOT rebind — a forwarded error page rebinding to *nothing* is precisely the divergence `assertStillBoundTo()` exists to catch.
- [2026-08-07 00:30] FOUND: **`TenantId::fromString()` NORMALISES a non-canonical id and `DocumentIdentity` REFUSES one**, and the asymmetry is correct in both directions. A document id is a key a client hands back, so two spellings compare unequal in an application-level comparison and refusing is right. A tenant id ends up in a TEXT GUC that the canonical policy casts with `::uuid`, and PostgreSQL's uuid comparison is case-insensitive — so normalising loses nothing and refusing would reject a caller who was not wrong. Found because a test asserted the opposite and failed; now pinned, because a reader who knows one of the two types would guess wrong about the other.
- [2026-08-07 00:30] RECORDED: **the new gate's first version reported its own subject's PHP docblock as configuration**, and its second missed a `//` comment whose knob was followed by a colon. Both are `worker-mode-blocked.sh`'s recorded false-positive class — "comment text was read as configuration" — arriving through comment syntaxes a `#`-only strip does not see. Comment handling is now decided ONCE into a variable every check reads, with three leaders. Also worth knowing: the meta-suite could not see the gate at all until it was `git add`ed, because its inventory uses `git ls-files` without `--others` — the split § Gotchas records the same day.
...
- [2026-08-06 21:30] AGREED: **the `InvoiceRepository` PORT TAKES NO TENANT, correcting a ruling that was not buildable.** `DocumentIdentity`'s docblock said *"the repository takes the tenant as an explicit argument instead, which is also what enforces the boundary rule"*. `TenantId` lives in `Infrastructure/Tenancy/`, so a `Domain/` port naming it is an outward dependency and a P0 [Verified: a probe interface declaring `f(TenantId $t)` produced *"Domain references Twes\Infrastructure\Tenancy\TenantId, which is outward"*, exit 1]. A bare `string` parameter would satisfy the gate and lose the point — that is a hole with a type, not a tenancy control. So the adapter is constructed with the request's `TenantContext` and refuses when none is bound. **Stronger rather than weaker**: a parameter is satisfied by whatever tenant id the caller happens to hold, including the wrong one, while a context resolved once cannot be forged at the call site — and the reductio § Gotchas 2026-07-31 applied to a FIELD on every type applies identically to a PARAMETER on every method.
- [2026-08-06 21:30] AGREED: **`save()` writes with DBAL, not through the UnitOfWork, and that is a measurement rather than a preference.** Whole-rewrite of the child rows through the `EntityManager` is not merely inefficient, it is IMPOSSIBLE: `remove()` on the line at position 0 plus `persist()` of a new line at the same composite primary key raises `Doctrine\ORM\Exception\EntityIdentityCollisionException` from the identity map, **before any SQL is emitted** [Verified against the migrated schema]. That is the same immutability/identity-map mismatch that put the mapping on a separate model in the first place. What the ORM mapping is FOR is now stated in the adapter's docblock rather than left as an apparent contradiction: it declares the schema `doctrine:schema:validate` checks, it types the rows the mapper produces, and it is what a future `migrations:diff` reads. It is not the write path.
- [2026-08-06 21:30] AGREED: **`save()` REFUSES outside an active transaction rather than opening one.** A document number is gapless, so allocating one and persisting the document that carries it must commit together or not at all; a repository that committed on its own behalf would make the atomic case unwritable, and the failure mode is a permanent hole in an invoice sequence. Refusing is also what makes the upsert-plus-delete-plus-insert sequence atomic without this class reasoning about nesting. Pinned by a case and a mutant.
- [2026-08-06 21:30] AGREED: **the port declares no `delete()` and no query methods.** An issued document is `cancel()`ed, never deleted — adding a delete would put a way to destroy a legal document beside the way to void one. Listing, filtering and pagination belong to a read model and API Platform's pagination extension, not to an aggregate repository, which loads and stores ONE consistency boundary at a time.
- [2026-08-06 21:30] FOUND: **a `Domain/` port gets NO auto-alias, so the binding must be explicit configuration.** Symfony auto-aliases an interface to its single implementation only when the INTERFACE is found in an autowired resource, and `Twes\Domain\` is deliberately excluded from autowiring. Without the alias in `services.yaml` the first `Application/` handler asking for `InvoiceRepository` fails with a message about an unresolvable argument rather than about a missing alias. [Verified: `debug:container` showed no alias before the line was added and the alias after.] This is the shape every future domain port will need.
- [2026-08-06 21:30] RECORDED: **the repository's own test caught me committing the exact defect its class docblock describes.** Two assertions compared a `quantity` by STRING and failed with `'2.000000'` against `'2'` — `NUMERIC(21,6)` returns the column's scale, not what was written — which is round 22's finding restated in the file that documents it, two paragraphs below the paragraph explaining it. The two halves are deliberately different: the in-memory mapper test asserts STRING identity (the mapper must not renormalise), and this one asserts NUMERIC equality (the database legitimately re-scales). Being able to state a rule is not the same as applying it.
...
- [2026-08-06 20:00] AGREED (decision 2 of 2): **the re-check is driven from the DBAL seam that emits the savepoint, not from repository code.** `SavepointTenantBindingMiddleware` + three wrappers, tagged `doctrine.middleware` on the `default` connection only. Chosen because it is the strongest option that EXISTS: the plan's first-ranked alternative — forbid savepoint-backed nested transactions in configuration — is not available in DBAL 4, where `setNestTransactionsWithSavepoints(false)` throws and `beginTransaction()` above level 1 creates a savepoint unconditionally [Verified in `vendor/doctrine/dbal/src/Connection.php`]. Decisive advantage over a per-repository call: DBAL issues a savepoint for you on a nested `beginTransaction()`, so the code that CREATES the divergence need not contain the word "savepoint" and its author has no cue to check. **The clause that followed — *"`autoconfigure: false` on the service is load-bearing — DoctrineBundle's autoconfiguration tags every `Driver\Middleware` for EVERY connection, and `debug:container` showed both `.default` and `.owner` before the explicit definition existed"* — is REFUTED, and this is the FIFTH site of it**, after § Gotchas claimed all three then all four were corrected. Its mutant SURVIVES: `MiddlewaresPass` builds its per-connection map by SKIPPING every `doctrine.middleware` tag carrying no `connection` attribute (`continue`, MiddlewaresPass.php:38-44), so the bare autoconfigured tag contributes nothing once a SCOPED tag exists and the two never compete. The `debug:container` observation was real and was made with NO explicit tag; extrapolating it to the case where one exists is what made it wrong. **`connection: 'default'` is the load-bearing part** [Verified: dropping the flag leaves `owner` unwrapped; dropping the *tag* installs the middleware on both]. The flag is kept for the smaller true reason — without it `debug:container --tag` lists a phantom second tag with an empty connection column, in the one command a reader would use to check the scoping.
...
- [2026-08-01 16:00] AGREED, **AND SUPERSEDED 2026-08-06 — API Platform IS INSTALLED AND IS THE HTTP SURFACE.**
  `api/composer.json` requires `api-platform/core: ^4.3` and `api/src/UI/Http/ApiResource/InvoiceResource.php`
  declares the operations. The ruling as originally written read *"API Platform is NOT installed and is scheduled
  NOWHERE [Verified: zero occurrences in `api/composer.json` and `api/composer.lock`, no `api/vendor/api-platform/`]"*
  — accurate on 2026-08-01, false from the commit that landed the READ path, and left standing for the whole of
  Wave 1's transport. Corrected in place rather than annotated below, per `CLAUDE.md` § Gotchas 2026-07-29, and kept
  rather than deleted because the REASON it was recorded is the part that still holds: it is named as the MANDATED
  mechanism by `CLAUDE.md` § "The Symfony ecosystem is the ONLY vocabulary" (API Resources → API Platform resources;
  Eloquent pagination → its pagination extension), and a mandated dependency appearing in no deliverable list is how
  a wave starts by hand-rolling the thing it was told not to. That is precisely what it stopped happening.
...
| **The i18n catalogues STILL have no consumer, and both halves of the old reason were false by 2026-08-07** (round 2, R2K-10). It said *"Nine keys in three locales"* — there are **35**, per `locale-key-parity`'s own output — and *"There is no HTTP layer to translate for"*, which `api/src/UI/` refuted three commits earlier. `CLAUDE.md` § "Translation keys" had the identical claim corrected in place by `1b5df38`; this second occurrence was not swept, which is the same append-don't-replace failure one file over. | The gap is real and unchanged: **nothing resolves a key**. A dozen keyed refusals raise a bare `\InvalidArgumentException` carrying an English sentence, so the transport cannot tell one from another — resolving them needs a TYPED EXCEPTION PER REFUSAL in `Domain/` carrying its key and placeholders, which touches every guard in the document kernel and is a larger deliverable than the endpoint that revealed it. What the write path does instead is stated in `CLAUDE.md` § "Translation keys": the validator translates the SHAPE errors in all three locales, and a domain refusal that survives becomes a 422 carrying English. Still owed, with a reverse gate (a key used in code but absent from a catalogue). |
...
## Wave 1 — Client & the invoice core — **THE WAVE'S SCOPE IS DELIVERED; IT IS NOT YET CERTIFIED** (domain 2026-07-31; schema 2026-08-01; repository, savepoint guard, boundary rule and the connection lifecycle 2026-08-06; the invoice HTTP surface 2026-08-06 read / 2026-08-07 write; the `e2e` suite 2026-08-07; the tenant settings table 2026-08-21; Client (+ contacts), Product and the `document` → `client` link 2026-08-22). **Nothing in this wave's scope is owed.** Client (+ contacts) and Product each landed with a domain aggregate, a tenant-owned RLS-policed table, a Doctrine adapter and a `POST`/`GET` surface (`PUT`/`DELETE`/collection-`GET` are deliberately absent from both, with their reasons on `ClientResource` and `ProductResource`), and the `document` → `client` link — `document.client_id`, the composite FK with `ON DELETE RESTRICT`, and the `document_client_required_once_issued` CHECK — landed at `de74d67`. **What separates this wave from COMPLETE is the MAXIMAL certification tier, not code**: the boundary rounds below have never reached two consecutive clean rounds and their OPEN rows are the outstanding balance. Do not read this heading as a licence to open Wave 2 — read the round tables. Each item was struck from this heading in the change that delivered it rather than left standing beside its own delivery, which is the discipline this line exists to enforce: it said `WAVE COMPLETE` for two commits after a retraction written 45 lines beneath it, and then said the `document` → `client` link was `Still owed` for one commit after `de74d67` delivered it — this file's append-don't-replace defect landing twice on the one line a reader and every table of contents render first.
...
1. **The Symfony application** — `api/src/Kernel.php`, `api/bin/console`, `api/config/**`, `api/public/`. Both
   ...
   *plugin* and this container's Composer configuration disables it. **The clause that followed — *"`bin/console`
   therefore uses the classic bootstrap rather than `vendor/autoload_runtime.php`, which `symfony/runtime` generates
   from that same plugin"* — was FALSE and is corrected in place** per `CLAUDE.md` § Gotchas 2026-07-29.
   `autoload_runtime.php` existed the whole time, `composer dump-autoload` regenerates it, and `symfony/runtime` is
   allow-listed; both `bin/console` and `public/index.php` require it today. The cost of believing otherwise was not
   stylistic — `autoload_runtime.php` is the ONLY mechanism by which `APP_RUNTIME` selects a runtime, so worker mode
   was not "not yet enabled", it was impossible. See `CLAUDE.md` § Gotchas 2026-08-02, and note that worker mode is
   now reachable and DELIBERATELY refused by `scripts/gates/worker-mode-blocked.sh` until Wave 10's portal token.
...
`DoctrineInvoiceRepositoryTest` against a real migrated schema. The mapper and its in-memory round-trip contract test
had landed at `b8d82c3`, hardened at `030550e`, and the rendered-number column the mapper was waiting on landed
...
non-empty, per its own instruction to add a layer as soon as it acquires code. The `functional` suite holds
**however many `git ls-files api/tests/Functional | grep -c '\.php$'` reports, and NO NUMBER IS WRITTEN HERE** —
this sentence said "three", then "five", then "six", going stale three times, and round 2's R2K-12 closure claimed
it *"now carries the derivation rather than a number"* while it carried BOTH: the derivation had simply been added
beside the latest count instead of replacing it, so the fourth staleness was already committed. That is round 3's
R3K-13, and the lesson is narrower than "counts drift" — **adding a derivation next to a number does not remove the
number, and a closure note is written by the author reading their new text rather than the sentence as it now
stands**; **`e2e` HELD NO CODE WHEN THIS WAS WRITTEN AND HOLDS `ServedSurfaceTest` SINCE 2026-08-07**,
...
it on the wave's own `In:` line, which names **Client (+ contacts)** and **Product** — neither of which existed in
any form when the retraction was written. **THE CITATION THAT STOOD HERE NO LONGER REPRODUCES, AND IT IS KEPT
VISIBLE RATHER THAN SWAPPED OUT**, because a `[Verified]` figure that has silently stopped being true is the
artefact this project most wants a reader to distrust — the same treatment § Gotchas gives the Composer
`without source` counts. It read:

> [Verified: 0 tracked files under `api/src/**` matching client/contact; the only `/Product` file is
> `Domain/Pricing/ProductPricing.php`, a pricing value object rather than an aggregate; the only tables any
> migration creates are `document`, `document_line`, `document_charge`, `document_number_sequence` and
> `messenger_messages`.]

**What is true on 2026-08-22:** the client/contact half is refuted — `Domain/Client/` holds four types, `client`
and `client_contact` are real policed tables, and `POST /api/clients` + `GET /api/clients/{id}` are served. The
Product half is refuted too as of the same day: `Domain/Product/` holds the aggregate and its port, `product` is a
real policed table, and `POST /api/products` + `GET /api/products/{id}` are served. What replaced the retracted
claim is NOT "the wave is complete" but a list that is now down to ONE item: the `document` → `client` link. The tenant SETTINGS table, which this
paragraph also recorded as absent, landed 2026-08-21 — `services.yaml`'s `$numberWidth: 7` and
`CreateInvoiceProcessor`'s `PerRateGroup` literal are both retired. Deliverable 2 (the migration template)
remains recorded as unmet by this very section.
...
| R1-2 | **Concurrent issue burns a number and rewrites a client's.** No row lock or version guard on the issue transition; every `FOR UPDATE` under `api/src/` is a comment | **P0** | **CLOSED 2026-08-07** — reproduced first with two live transactions (`allocated=[1,2] on documents=[2]`), then closed twice over: `InvoiceRepository::findForMutation()` (a new port method stating a *guarantee*, delivered by `FOR UPDATE`, taken BEFORE the allocation because the counter row is itself contended) and — the half that actually closes the hole — a write-once number predicate on `save()`'s upsert that refuses a renumber whatever read produced it. Six mutants: bind the lock away, lock on every read, delete the predicate, delete its rowCount check, narrow it so a cancellation is refused, revert the handler to `find()`. The composed end-to-end case does NOT pin the lock and says so — see § Gotchas 2026-08-07 for why, and for the second process it would take |
...
| R2K-7 | completeness | `CLAUDE.md`'s hand-written gate block lists 14 of 15 gates; `no-forgeable-tenancy-in-production.sh` is absent | P2 | **CLOSED** — added, and the derive-this-block note now records that the block has been wrong TWICE, both times omitting a SECURITY gate, the second time inside the commit that had just rewritten every line of it |
...
| R2K-9 · R2C-8 | completeness, correctness | The `TWES_SCHEMA_USER` relaxation has no case in either suite, and its predicate misses a DSN whose FIRST parameter is `user=` | P2 | **CLOSED** — the predicate now PARSES the DSN (strip the scheme, split on `;`) instead of guessing separators, and three meta-cases cover user-last, user-first and an EMPTY `user=` which must not count. Mutant: restore the shipped regex → `491 passed, 2 failed` |
| R2C-7 | correctness | `InMemoryInvoiceRepository`'s docblock cites `IssueInvoiceHandlerTest`, which does not exist | P3 | **CLOSED** — the class is `InvoiceWriteHandlersTest` |
| R2K-10 | completeness | The i18n row still said there is no HTTP layer and counted nine keys (35) — the identical claim `1b5df38` corrected in `CLAUDE.md` and did not sweep for here | P2 | **CLOSED** — corrected in place, and the row now states WHY the gap is still open (a typed exception per refusal is needed first) rather than merely that it is |
| R2K-11 · R2K-12 | completeness | Two stale counts: "two rounding constants" (one), and "FIVE functional classes" (six) | P3 | **R2K-11 CLOSED. R2K-12 was NOT** — this cell claimed the sentence "now carries the derivation rather than a number" and it carried both, the derivation having been added BESIDE the count rather than replacing it. Reopened and genuinely closed at round 3 as R3K-13 |
...
### Wave 1 boundary — round 4 (MAXIMAL, frozen `626fc33`): 25 findings, 0 P0, NOT clean

Recorded AS FILED, per R2K-8, and **written before any of it was closed** — round 3's record had to be reconstructed
from closure commits afterwards, which is the drift R2K-8 exists to stop. Same isolation harness. Zero P0s for the
second round running. **The two-consecutive-clean counter stays at ZERO.**

**ONE FINDING WAS REACHED BY TWO LENSES** from opposite directions — R4S-5 ≡ R4K-11, one from the code and one from
`CLAUDE.md`'s claim about it — so 25 filed IDs are 24 distinct defects.

**The negative record is the largest of any round here and is part of the result, not padding.** The tenancy lens ran
a full attack matrix against a live migrated database as the restricted role (unbound read, cross-tenant read, insert,
update, re-parent, delete, `TRUNCATE`, uniqueness oracle, FK reference, `COPY … TO PROGRAM`, temp tables, `pg_authid`,
`pg_read_file`, owner-owned views, `SECURITY DEFINER` functions), full-HTTP IDOR through the real kernel in both draft
and issued states, the `PGOPTIONS` pin in two timings, savepoint divergence, and `COMMIT AND CHAIN` — **all refused** —
plus four mutants confirming earlier closures still hold. The correctness lens killed 24 mutants across the money,
rounding, allocation, state-machine and mapper surfaces. The completeness lens reconciled translation keys, the
`TWES_TEST_DB_*` set, ports vs adapters vs doubles, and the tenancy guard's ten public assertions against their
wiring — **nothing in that delta unwired** — and established cross-tier reach as genuinely moot rather than assuming
it. Each lens also stated what it did NOT attempt, rather than letting silence read as coverage.

| # | Lens | Finding | Sev | Status |
|---|---|---|---|---|
| R4C-1 | correctness | **The child-row comparison that closed round 3's triple-hit compares `vat_rate` at scale 6 while the column is `NUMERIC(27,12)`, so a changed VAT rate is STILL silently accepted and discarded** — and the comment's reason (*"scale 6, the widest of the decimal columns involved"*) is false, `vat_rate` being the widest at 12. An issued document stored at 19 % re-saved at 19.00001 % compares EQUAL, the state-only branch returns success, and nothing is written — the *"ignoring a change is not refusing one"* behaviour that branch exists to eliminate. The rate is on the wire at ten decimals, so the two are visibly different documents | **P1** | **CLOSED** — `COMPARISON_SCALE`, anchored to `Rate::FRACTION_SCALE` rather than written as `12`. The property is restated as **"at least as wide as every decimal column compared"**, not "the widest of them": over-shooting can never be wrong (a column cannot hold a digit past its own scale, and `bccomp` treats trailing zeros as equal) while under-shooting fails SILENTLY. Pinned by a case varying ONLY the rate; mutant restoring 6 → 1 failure |
| R4C-3 | correctness | **The per-line VAT on the wire being the ALLOCATED share is enforced by a comment and nothing else.** Replacing `$vatByLine[$position]` with a recomputed `net × rate` survives both suites. Not an equivalent mutant — two 0.013 TND lines at 19 % give `[0.003, 0.002]` allocated against `[0.002, 0.002]` recomputed, so a millime the document owes appears on no line. Root cause is the FIXTURE: `tndInvoice()` has zero allocation remainder, so the two implementations are byte-identical there | **P1** | **CLOSED** — a `remainderInvoice()` fixture and a case asserting the exact shares (which also pins the earliest-line tie-break). Its own comment anticipated a DIFFERENT vacuity and missed this one |
| R4C-4 | correctness | **Computing the totals with the document's persisted `vatRoundingPoint` is unpinned; only the emitted LABEL is asserted.** Hardcoding `PerRateGroup` for the computation survives both suites, serving a `per_line` document with `per_rate_group` figures while declaring `per_line`. The label's own docblock makes it a claim ABOUT the figures. Latent today, live the moment the settings table lands — which is why the value is persisted per document | **P1** | **CLOSED** — the same fixture; the rounding-point case now asserts `0.005/0.031` against `0.004/0.030`. One fixture killed both mutants, which is what identified them as one defect |
| R4C-2 | correctness | A document-TOTAL overflow answers **500**, not the `document.total_too_large` 422 that § "Translation keys" keys, because the comment placing `handle()` outside the `try` (*"`Invoice::draft()` plus `withLine()` cannot refuse anything the conversion above already accepted"*) is false — the per-line check is not the document check. Measured through a real kernel: per-line overflow 422, two-line SUM overflow 500, charge overflow 500. The fourth member of the try-boundary family in this one file | P1 | **CLOSED 2026-08-23** — `DocumentCannotBeTotalled` (extending `\InvalidArgumentException`, so every `@throws` tag on the three mutators stays true and no existing `catch` changes) is thrown by `Invoice::totallable()` and caught in its OWN arm in `CreateInvoiceProcessor`, the shape `UnknownClient` established — NOT a wider catch, which would re-admit the mapper's corrupt-column `\InvalidArgumentException` that must stay a 500. Three cases added to `everyRefusalACallerCanFix`, red first for the stated reason (uncaught `\InvalidArgumentException` at `CreateInvoiceProcessor.php:133`). TWO mutants, each 3 errors: narrowing the catch back to `UnknownClient` alone, and reverting the domain to the bare throw. The false comment is corrected in place. `assertRoomFor()` deliberately gets NO arm and the reason is recorded at the call site: `#[Assert\Count(max: Invoice::MAX_LINES)]` on both collections makes it unreachable from this transport |
| R4K-1 | completeness | `infra/README.md` tells an operator `/api/docs` returns 404 and that no HTML documentation UI ships. Both halves false: `enable_swagger_ui: true`, `html` in `docs_formats`, a dedicated `@apiDocs` Caddy matcher, and an `e2e` case on the path. The state before 2026-08-05 — the three-failure fix § Gotchas records — left uncorrected | P1 | **CLOSED 2026-08-23** — the row now says `/api` in a BROWSER serves the Swagger UI page, and names what the old one got wrong twice over: not only that `/api/docs` 404s and no HTML UI ships (both false since 2026-08-05), but that its privacy reasoning had the UIs backwards — Swagger UI's assets are LOCAL, and it is ReDoc that reaches `cdn.redoc.ly`, which our own `img-src 'self' data:` blocks |
| R4K-3 | completeness | `DatabaseRequirement` states **"THERE IS NO LEGITIMATE SKIP IN THIS SUITE"** and there are eight, seven gating mutant-killing security cases behind a missing env var. Round 15's fail-rather-than-skip ruling reached `superuserConnection()` and nowhere else, so a CI setting the superuser pair but omitting the probe-role names reports green with seven controls unexercised | P1 | **CLOSED 2026-08-23** — all EIGHT converted to `self::fail()`, each naming the missing variable, the control left unexercised and the command that provides it. Measured BEFORE, not inferred: stripping `TWES_TEST_DB_PROBE_OWNER_ROLE` reported `OK, but some tests were skipped!` with `Tests: 3, Skipped: 3`; stripping `TWES_TEST_DB_TRUNCATOR_ROLE`, `Skipped: 1`; an unreachable `postgres`, `Skipped: 1` and a measured exit **0**. AFTER: `Failures: 3`, `Failures: 2`, `Failures: 1`, exit **1** in all three. (The first two BEFORE probes report the TALLY rather than an exit code on purpose — their status was captured through a `tail` pipeline, so what `$?` held was `tail`'s, the § Gotchas 2026-08-01 defect one level down. `OK, but some tests were skipped!` is exit 0 by PHPUnit's own semantics and by the 2026-07-30 incident this suite exists because of, but that is an entailment, not a measurement, and the BEFORE state no longer exists to re-measure.) **The finding UNDERSTATED it in one direction and OVERSTATED it in another**, and both corrections are the useful part. Understated: the eighth is not an env guard but an unreachable-database arm, and it guards the NULL-`datacl` DANGEROUS DEFAULT whose own docblock records a mutant that *passed the whole suite* — so it is converted too, with a message saying what it would TAKE rather than declaring the arm uncoverable (§ Gotchas 2026-07-30). Overstated: **three of the seven were already dead code** — the guards on `TWES_TEST_DB_USER` are shadowed by `setUp()`'s own `connectAs()`, which fails first, so stripping that variable produced `Failures: 7` before any change. They are converted rather than deleted: a guard removed because today's only caller cannot reach it is removed for the wrong reason. **Plus the durable half the finding did not ask for**: `NoLegitimateSkipTest` scans every tracked integration file for a CALL (not a mention — both files that STATE the rule name the method in prose, and exempting them is the exemption-inside-a-cross-check shape), enumerating from `git ls-files` per § Gotchas 2026-07-31, with its own anti-vacuity halves in both directions and NO self-exemption, which is possible because the method name is assembled rather than written. Mutant: reintroducing one skip turns `OK (3 tests, 29 assertions)` into `Failures: 1` naming `TenantIsolationTest.php:983`; restored and re-verified green. **Five stale prose sites corrected in place** — round 15's ruling reached `superuserConnection()`'s own docblock and left `connectAs()`, the `tearDown` guard, the predefined-role comment, the SECURITY DEFINER docblock and the proconfig docblock all still describing these tests as skipping |
| R4S-5 · R4K-11 | tenancy, completeness | A tenant-less request answers an untyped **500**, not a keyed 4xx — an authorization-shaped refusal reported as a server fault, i.e. the one signal that would show a tenancy control failing. § "Translation keys" lists `error.tenant_required` as *"a reachable answer of the live transport"*; the reachable answer is a bare `\RuntimeException`. No data leaks, but every unauthenticated call is an indistinguishable 500 in logs, and Wave 7 inherits this path | P2 | **CLOSED 2026-08-23, at ALL FIVE SITES.** `Twes\Infrastructure\Tenancy\Exception\NoTenantBound` extends `\RuntimeException` — narrowing, so every `@throws` tag and any existing `catch` stays true — and `api_platform.exception_to_status` maps it to **401**. The five were found by grepping the RULE rather than by following the finding, which named one: `DoctrineInvoiceRepository`, `DoctrineClientRepository`, `DoctrineProductRepository`, `DoctrineCompanySettingsRepository` and `PostgresDocumentNumberSequence`. Each KEEPS ITS OWN MESSAGE — a repository explains that a tenant-less read is indistinguishable from *"there is nothing"*, the sequence explains that a number is per (tenant, type) — because collapsing them would trade a precise diagnostic for a shorter constructor. **401 not 403**, argued at the config: the request has not proved WHO it is, and 403 would claim an identity to deny that does not exist before Wave 7. Measured both ways: `TenantlessRequestStatusTest` passes at 401, and reverting ONE site to a bare `\RuntimeException` gives `Failed asserting that 500 is not identical to 500`. **The key is CARRIED, not RESOLVED**, and § "Translation keys"'s *"None of them RESOLVES its key yet"* is amended in place to say so rather than left to contradict this — the typed-exception-per-refusal catalogue stays owed. Two defects of my own on the way, both kept: the scripted rewrite left a no-op `\sprintf('%s', $attempted)` at four sites, and it merged `PostgresDocumentNumberSequence`'s THREE-condition `@throws` into one tag claiming `NoTenantBound` for two conditions that still raise a plain `\RuntimeException` — php-cs-fixer's realignment is what made that run-on visible |
| R4S-1 | tenancy | `TWES_TRUST_TENANT_HEADER=0` in `infra/.env` is referenced by no compose file, Dockerfile or entrypoint — a compose INTERPOLATION variable that never enters a container. **R3S-4's exact shape, on the line immediately below the one R3S-4 fixed, in the same file.** Two source sites and `services.yaml` call it one of "three independent controls"; three of three is two of three, and the missing one covers the deployment the docblock singles out as riskiest. Fail-SAFE — `api/.env` ships in the image carrying `0` — so the defect is the claim, not an open door | P2 | **CLOSED 2026-08-23, and the fix is STRONGER than the sibling's.** Wired into `x-api-environment` as a **LITERAL `'0'`**, not `${TWES_TRUST_TENANT_HEADER:-0}` — because the obvious spelling, matching the `TWES_ASSERT_REVOKED_CAPABILITIES` line directly above it, is REFUSED by `no-forgeable-tenancy-in-production.sh` [Verified: `violations=1`], whose polarity rewrite requires the single permitted value rather than a value assembled from two files. The refusal is right: an interpolation would make `infra/.env` AUTHORITATIVE, so editing the deployment's own dotenv would switch on a header that lets any caller act as any tenant. The literal closes that seam entirely. [Verified: the value now renders into **5 services in each of the two configurations** as `"0"`, where it previously reached none.] **Plus the rendered assertion**, property 8 in `compose-config.sh` — and it is demonstrably NOT redundant with the text gate: a gitignored `.env.*.local` reached through `env_file:` leaves `no-forgeable-tenancy-in-production.sh` reporting **OK with 0 violations** while property 8 catches it on two services. (The first mutant tried — flipping the anchor to `1` — fires property 8 but is one the text gate also sees, since its `--cached --others` scope includes untracked files; the gitignored case is the one that separates them.) The *"three independent controls"* claim is swept at every site `git grep` finds — `CLAUDE.md`, `api/.env`, `infra/.env`, `api/config/services.yaml` — and no count is written at any of them |
| R4S-2 | tenancy | `10-roles.sh` claims `compose-config.sh` cross-checks its `REVOKE` block against `LARGE_OBJECT_WRITERS`; **nothing does** (the only match in `scripts/` is prose in an unrelated error message). Adding a seventh real writer to the constant leaves every gate and all 986 tests green — and then **refuses every connection acquisition in api, worker and scheduler**, a total outage on the next edit. `10-roles.sh` is executed by no test and read by no gate, unlike its dev twin | P2 | **CLOSED 2026-08-23 by IMPLEMENTING the claimed check rather than amending the claim**, which is how R3S-4 closed and is the difference between a whole closure and half of one. `10-roles.sh` said *"`compose-config.sh` asserts the two agree"*; nothing did, and the only match for the constant under `scripts/` was prose in an unrelated message. The check lives where the claim already pointed, compares **BY FUNCTION NAME** (the REVOKE block names `lo_import` twice, once per signature, so a raw-line comparison would report a phantom difference forever), runs in **BOTH directions**, and **fails rather than passing when either side is unreadable**. **It sits ABOVE the `docker compose` probe and therefore never skips** — a text comparison needs no daemon, and `10-roles.sh` is executed by no test and read by no other gate, so a skipping check would leave it unverified on exactly the machines that cannot run the stack. That changes the gate's documented contract, so **both statements of it were updated in the same change** (the gate's own header and its skip message, plus `CLAUDE.md`'s command block): the RENDERED half skips without Docker, this half does not. Two mutants: a seventh name in the constant is reported as *checked, never revoked*; a seventh `REVOKE` is reported as *revoked, never checked*, naming `lo_unlink` in each |
| R4S-3 | tenancy | R3S-5's fix is pinned on one of its two short-circuits: moving the per-session assertions below the **in-kernel** guard survives the whole suite, while moving them below the **pool** hit is killed by exactly one test. The in-kernel array is the one R3S-7 showed has no TTL. Honestly not a breach — the reviewer drove the real `PGOPTIONS` vector under the mutant and the pin was still refused, by `bind()`'s own backstop; the second per-session assertion has no such backstop | P2 | **CLOSED 2026-08-23.** The reason the in-kernel short-circuit was unpinned is structural rather than a missing assertion: `guardedConnection()` builds a NEW `ConnectionProvisioningGuardMiddleware` per call, so `$verifiedInThisKernel` is empty every time and that branch is never crossed — the pool case cannot reach it. `testAWarmInKernelEntryDoesNotLetAPinnedSessionThrough` shares ONE middleware across two acquisitions, which is what a resident worker does, and drives the real `PGOPTIONS` vector on the second. It is the more dangerous branch of the two, because that array sits IN FRONT of the pool and has no TTL (R3S-7), so in the worker and scheduler the window is the process's life rather than `$verificationTtl`. Both orderings are now pinned: moving the per-session assertions below the IN-KERNEL guard gives `Failures: 1` where the finding measured it surviving the whole suite, and below the POOL hit gives `Failures: 2` where it previously cost exactly one test |
| R4C-5 | correctness | The `count()` guard is the only thing detecting a save proposing FEWER child rows than are stored, and nothing in the tree proposes one — `childRowsAgree()` iterates `$incoming`, so a shorter incoming set compares only its own rows and returns `true`. The `?? null` covers the opposite direction only | P2 | **CLOSED 2026-08-23** — the guard was CORRECT and genuinely unpinned, confirmed before writing anything: replacing `\count($stored) !== \count($incoming)` with `if (false)` left the integration suite green at `OK (286 tests, 1140 assertions)`. `testASaveProposingFewerChildRowsThanAreStoredIsRefused` stores TWO lines and ONE charge so that "fewer" is expressible on each child table, and proposes a shorter set whose surviving rows are byte-identical to the stored ones — so only the count can refuse it. TWO mutants, because the two `childRowsAgree()` calls are separate and either could be dropped alone: `if (false)` on the count kills the LINES arm, and replacing the charge comparison with `false` kills the CHARGES arm. It also asserts both child sets SURVIVED the refusals, since the write path `DELETE`s them with no predicate of their own |
| R4C-6 | correctness | Per-line VAT is computed on every path, so `PerRateGroup` + `RoundingMode::Unnecessary` refuses a document whose per-rate-group VAT is exactly representable (two TND lines of `0.001` at 50 %: group VAT `0.001` exactly, each line's share `0.0005`). Unreachable today — every caller uses `HalfUp` or `Up` — but `totals()` is public and documented as taking the caller's mode | P2 | **CLOSED 2026-08-23 — reproduced, and it was a real refusal of a correct document.** Two TND lines of `0.001` at 50 % (group base `0.002`, group VAT `0.001`, exact in a three-decimal currency) were refused with `Result "0.000500000000000" does not fit TND … RoundingMode::Unnecessary forbids rounding it`, thrown at `DocumentCalculator.php:117`. The per-line `net × rate` is DISCARDED under `PerRateGroup` — the shares come from `allocate()` — so the document was refused for a figure it never uses. Fixed by computing it under `PerLine` ONLY. The old comment's defence (*"branching means the two arms exercise different code and the cheap one rots"*) is quoted and answered in place: both rounding points are driven by the shared vectors, so neither arm goes unexercised. **Deliberately NOT fixed by softening the mode**, and a companion case pins why — under `PerLine` the same document must STILL be refused, because there the figure IS the answer. TWO mutants, both measured after a first attempt at the second was vacuous (a `git restore` had reverted the fix along with the mutant, so the "mutant" run was the unfixed baseline): making it unconditional again gives `Errors: 1`, making it never run gives `Errors: 18, Failures: 1` |
| R4C-7 | correctness | A false absolute — *"ZERO ROWS WRITTEN MEANS ANOTHER TRANSACTION ISSUED THIS DOCUMENT … Nothing else can produce it"* — contradicted by R3C-7's own type predicate fourteen lines above it in the same statement, whose comment explicitly describes that predicate failing. The pre-read carries no `type` predicate, so for a draft of another type at the same key the thrown message says *"it already carries a different document number"* about a row carrying **no number at all** | P2 | **CLOSED 2026-08-23 — reproduced against the live schema before any fix.** A quote DRAFT INSERTed at the identity an invoice `save()` then used produced, verbatim: *"it already carries a different document number"* about a row whose `number` is NULL, prescribing `findForMutation()` for a race that is not happening. The remedy is not a reworded string: the cause is now ESTABLISHED by re-reading `type, number` on the error path and branching, because inferring which arm fired from the failure itself is the derivation-that-agrees-with-itself shape § Gotchas 2026-07-31 records as a P0. Three arms — a TYPE collision (naming both types, and saying plainly that `findForMutation()` cannot help), a renumber (now quoting the number it actually read), and a row that has become unreadable between the upsert and the re-read, which says that nothing can be asserted rather than picking the likelier arm. The false absolute is corrected in place. Mutant: deleting the type branch turns the new case red, and the fallback then reports *"document number none"* — honest, and visibly the wrong diagnosis |
| R4K-2 | completeness | `infra/README.md` calls `/api/currencies` *"the one implemented resource"*; there are two resources and five operations | P2 | **CLOSED 2026-08-23** — the row names all five resources and sends the reader to `api/src/UI/Http/ApiResource/` or the OpenAPI document rather than to a count in prose, which is what went stale |
| R4K-4 | completeness | `no-forgeable-tenancy-in-production.sh` — a SECURITY gate in `gate:architecture` — is routed by the write-time hook for no file type AND is absent from that hook's "deliberately not here" list, so no decision is recorded either way. It fell through because the hook suite's coverage check is **one-directional**: it asserts only that the three excluded gates stay out, never that a non-excluded gate is in | P2 | **CLOSED 2026-08-23, both halves.** The gate is now routed on the CONFIG surface — dotenv, compose, YAML (`services.yaml` is where the knob is wired), Dockerfiles, shell and tracked JSON — and NOT on `*.php`, with the reason recorded at the routing rather than left implicit: this hook's own note says *"over-routing costs milliseconds"* and that premise does not hold here, the gate being **5.2s / 6.4s / 11.6s** over three runs [measured], the most expensive in the set. `composer gate` still runs it over everything tracked, so a `.php` assignment is caught before a commit. **The durable half is the one the finding really asked for**: the hook suite's coverage check was one-directional (it asserted only that three named gates stay OUT), so it is now BIDIRECTIONAL and derived — `git ls-files -- scripts/gates` minus `lib/`, every member either in `EXCLUDED_FROM_WRITE_PATH` or invoked. The exclusion set is decided ONCE into a variable both directions read, per § Gotchas 2026-07-29's one-guarded-write-path lesson, and `shell-syntax.sh` joins it with its reason (redundant — `lint-on-write.sh` already runs `bash -n` on the file written). 28 cases → 40. Mutant: un-routing the gate gives `39 passed, 1 failed` naming it |
| R4K-5 | completeness | `CLAUDE.md`'s *"every gate answers `--dump-rules`"* is false for two of fourteen, and both treat the flag as a normal run rather than refusing it, so a consumer cannot distinguish "no rules" from "ran the whole gate". `test-gates.sh` reads the flag from eight gates BY NAME and never asserts the universal — the hand-picked-versus-derived split that row is itself arguing against | P2 | **CLOSED 2026-08-23.** The two holdouts were `shell-syntax.sh` and `locale-key-parity.php`, and neither mentioned `--dump-rules` anywhere — each ran the whole gate and printed its ordinary verdict, so flag and no-flag produced BYTE-IDENTICAL output. Both now dump real rule data (the discovery rule and the check for the first; the translations root, the required locales and the owed tiers for the second). **The universal is asserted by DERIVATION** — over `gates_on_disk`, the same `git ls-files` set the clean-case and `composer gate` checks already use — replacing nothing, since the eight named reads up in the GENERATED-CASES block were never the assertion. The test is on BEHAVIOUR, not on a `grep` for the flag in the source, which would have passed a gate that parses it and ignores it: a dump must exit 0, emit something, and NOT carry a verdict line (`<name>: OK/FAIL/owed`, `counts —`), because rendering a verdict is what doing the job looks like. **A first version compared the dump against a second, PLAIN run** — the honest formulation, and it doubled every gate's runtime and took the suite past 12 minutes, so it was replaced by the verdict test, which costs one run. Anti-vacuity in both directions: reverting the two gates to their pre-fix state makes the check name both as `ignored-the-flag` |
| R4K-6 | completeness | Three authored `.xlf` catalogues carry no SPDX identifier (invariant 8(c)), `api/translations` is under no search root, `xlf` is in no documented exclusion — and the only trace of a decision is a `grep -v '\.xlf$'` proven to be **dead code** (replacing it with `cat` gives a byte-identical result). Every other absence from `EXTENSIONS` has a reason recorded beside it | P2 | **CLOSED 2026-08-23.** All three catalogues carry the identifier, `xlf` is in `EXTENSIONS` and `api/translations` is a search root — and **both dead `xlf` filters are DELETED**, with the reason they were dead recorded where they were: `xlf` was never in `EXTENSIONS`, so neither the in-root `case … *.xlf) continue` nor the coverage check's `grep -v '\.xlf$'` could ever fire, which is why replacing the latter with `cat` had been byte-identical. The count moved **280 → 283**, which is the tell § Gotchas 2026-08-05 prescribes: an unenforced convention is not a convention, and the cheapest way to find what a gate is not looking at is to make it look at one more thing. Detector before repair: stripping the identifier from one catalogue gives `MISSING SPDX header: api/translations/messages.fr.xlf` and exit **1** (measured with the pipeline removed — reading `$?` through a `tail` gives `tail`'s status, § Gotchas 2026-08-01). `locale-key-parity` still OK, 57 keys across en/fr/ar |
| R4C-8 | correctness | `withLine()`/`withFixedCharge()`'s currency guards are deletable with both suites green — the shape round 14 closed one file over and did not sweep back here. Honestly bounded: with the guard gone the mismatch is still caught downstream by `resolveCurrency()`, raising the same class with an arguably better message, so the finding is that the guard is unpinned and its stated rationale not load-bearing | P3 | **CLOSED 2026-08-23** — confirmed unpinned first (neutering both guards left `OK (805 tests, 2721 assertions)`), and the REASON is worse than "unpinned": the guard's comment claimed the calculator would catch it *"eventually"*, and `self::totallable()` runs at the bottom of the same method, so it catches it in THAT call — with the BETTER message, since it appends the locator `(document line 0)`. Both facts measured. **KEPT rather than deleted**, and the reason is stated at the call site: `totallable()` landed on 2026-08-23 for an unrelated finding (R4C-2), and deleting a guard that predates it would make an older invariant depend on it with nothing asserting that every mutator ends in a total. Six mismatch shapes were checked to confirm no hole either way. Pinned by `testTheEditTimeCurrencyGuardRefusesBeforeTheCalculatorDoes`, whose discriminator is the ABSENCE of a `(document ` locator. **The first version of that test was half-vacuous and the per-guard mutants caught it**: the charge arm asserted on `'fixed charge'` while the calculator says `document charge`, so it held whichever refusal arrived and the charge mutant SURVIVED a case written to kill it. Both arms now use the one measured discriminator, and both single-guard mutants kill |
| R4C-9 | correctness | A comment names `uasort` where the code calls `usort`, and the explicit position tie-break is deletable with both suites green. Judged an **equivalent** mutant (input is already in ascending position order and PHP 8's sorts are stable, exactly as the comment says), so the finding is the wrong function name on a claim about a tax figure | P3 | **CLOSED 2026-08-23** — the name corrected to `usort`, and the equivalence RE-MEASURED rather than taken on the finding's word: deleting `?: $a <=> $b` leaves the `unit` suite green at `Tests: 805`, so the mutant is equivalent TODAY and the clause is kept with that reason stated at the call site. The first draft of the correction asserted `uasort` would have sliced differently — it would not (`array_slice()` reindexes numerically by default; both spellings slice to `[1,2]` on the same comparator), and the wrong claim was caught by running it. The finding is exactly what it said: a sentence justifying a tax figure named a function the code does not call |
| R4S-4 | tenancy | The savepoint predicate is anchored at `^`, so a leading comment, a preceding statement or `/* retry */` reverts the binding without the re-check firing — reproduced on a live connection, four spellings, three unmatched. Labelled UNPROVEN as an exploit: the only source of a divergence today is a session pin, refused at both `connect()` and `bind()`. Filed because it is the enumerate-the-accepted-spellings polarity shape `worker-mode-blocked.sh` needed three defeats to learn | P3 | **CLOSED 2026-08-23 by inverting the polarity, not by adding spellings.** Reproduced first — the anchored `/^\s*ROLLBACK.../` misses `/* retry */ ROLLBACK TO SAVEPOINT sp1`, `-- retry\nROLLBACK TO SAVEPOINT sp1` and `SELECT 1; ROLLBACK TO SAVEPOINT sp1`, because `\s*` enumerates the ONE thing allowed to precede the keyword. **Over-firing is safe here and that was CHECKED rather than assumed**: `assertStillBoundTo()` is a pure no-op on an intact binding, returning quietly both when a tenant is bound and matching AND when none is bound and the GUC is empty, so a spurious match costs one `current_setting()` read while a missed one costs a connection silently scoped to a previous tenant. Comments AND single-quoted literals are stripped, then the match is unanchored. **The literal strip is not decoration**: this class's own suite pins `SELECT 'ROLLBACK TO SAVEPOINT' AS label` as NOT a rollback, and de-anchoring without it would have broken that — over-firing being safe is a reason to tolerate a false positive, never to introduce one. The grammar half is untouched, so a full `ROLLBACK` (which legitimately discards the binding) still does not fire. Two mutants: restoring the anchor and removing the literal strip each give `Failures: 1` |
| R4K-7 | completeness | A `[Verified]` mutant citation in `CLAUDE.md` and in the hook suite whose totals no longer reproduce (`28 passed` now, `27` claimed) — in the file whose own § "Claude config" says of that suite *"it reports its own case count; none is written here"*. Mechanism intact, mutant still kills; only the numbers are stale. The reviewer first mis-applied the mutant and briefly read the survival as a P1, which is worth keeping | P3 | **CLOSED 2026-08-23** — both sites now state the DIRECTION and no totals (*"exactly one fewer pass and one failure"*), re-measured: the suite reports **28 passed**, and the mutant gives **27 passed, 1 failed**, flipping the orphaned-docblock case as documented. Mechanism intact; only the numbers were stale. Restored and re-verified green |
| R4K-8 | completeness | The § Gotchas 2026-08-06 table partitioning gates by `git ls-files` mode names two gates in the `--cached --others` row; there are three. Same hand-written-enumeration cause as R4K-4, one level up: the third gate landed 2026-08-05, the table was written 2026-08-06 | P3 | **CLOSED 2026-08-23** — the third gate (`no-forgeable-tenancy-in-production.sh`) is named, and the row now carries the DERIVATION (`grep -n 'ls-files' scripts/gates/*.sh scripts/gates/*.php`, read which carry `--others`) rather than only a longer hand-written list, plus the note that the table was stale on the day it was authored. No count is written in the row |
| R4K-9 | completeness | The functional-suite description partitions "it" into two named groups covering four of six classes — and the omitted one is `TenantBindingWiringTest`, the one § Gotchas 2026-08-07 calls indispensable (*"Both, or neither is enough"*). The R2K-12 shape recurring on the sentence that replaced it | P3 | **CLOSED 2026-08-23, and it was WORSE than filed** — the partition named *"the two processor classes"* of six while the suite now holds **thirteen** classes in **three** kinds. Replaced by the kinds (`*ProcessorTest`/`*ProviderTest` drive the processor directly · `*SurfaceTest` needs the real serializer, validator and router · `*WiringTest` asserts what the CONTAINER did) with `TenantBindingWiringTest` named as the omitted third kind and § Gotchas 2026-08-07's *"Both, or neither is enough"* quoted. Membership is `find api/tests/Functional -name '*Test.php'`; no count |
| R4K-10 | completeness | `CLAUDE.md` and `scripts/claude-bootstrap/CLAUDE-global.md` disagree about `.claude/settings.json`: one says `deny: []` AND `ask: []`, the other verifies there is no `ask` key. No behavioural difference (absent ≡ empty), but the intent sentence reads as describing a key that is there | P3 | **CLOSED 2026-08-23 — half its premise had already died.** `scripts/claude-bootstrap/CLAUDE-global.md` existed at the round-4 freeze `626fc33` (10 tracked files) and is GONE from `HEAD`, removed 2026-08-18, so there is no second document left to disagree. The surviving half is real and fixed in place: *"Both `deny` and `ask` stay empty"* described an `ask` key that is not there. The sentence now states the two SPELLINGS — `deny` present and empty, `ask` absent altogether — because those are what a reader compares when checking the claim. [Verified: `jq '.permissions | {has_deny: has("deny"), has_ask: has("ask"), deny}'` → `true`, `false`, `[]`.] |

### Wave 1 boundary — round 3 (MAXIMAL, frozen `7d4cfbc`): 29 findings, 0 P0, NOT clean

Recorded AS FILED, per R2K-8. Same isolation harness as round 2 — a `git archive` of the frozen commit per lens,
each with its own `COMPOSER_VENDOR_DIR` (a `cp -al` hardlink farm plus a real copy of `vendor/composer`, so
`dump-autoload` could not reach ours) and its own git index; only the tenancy lens was permitted the `integration`
suite and `pg_ctlcluster`, because the probe databases have fixed names. **Zero P0s, and that is the first round of
this wave where the panel found no blocker.** It is NOT a clean round: the two-consecutive-clean counter stays at
**ZERO**, and the next round is the first that can begin counting.

**ONE FINDING WAS REACHED BY ALL THREE LENSES INDEPENDENTLY** (R3S-6 · R3C-1 · R3K-6), and two more by two each.
That is the return on a panel rather than a single reviewer, and it is the third round running in which the
triple-hit was a real defect rather than a shared misreading.

| # | Lens | Finding | Sev | Status |
|---|---|---|---|---|
| R3S-1 · R3K-14 | tenancy | **A REPRODUCED CROSS-TENANT TRUNCATE.** `ALTER DEFAULT PRIVILEGES … GRANT` is ADDITIVE, so a pre-existing `GRANT ALL ON TABLES` survived a corrective run of `provision-dev-database.sh` and the runtime role kept TRUNCATE on every table the NEXT migration creates — while the script printed *"cannot CREATE, TRUNCATE or own"*. The panel truncated a tenant-owned table as the restricted, non-superuser, non-`BYPASSRLS` role, destroying a row the session could not see. R3K-14 is the same reset naming ONE grantee, so `GRANT TRUNCATE … TO PUBLIC` survived it — a PUBLIC grant reaches the runtime role while appearing in no grant TO it, so an audit of the role reports clean | P1 | **CLOSED `849a52d`** — both axes REVOKE before they GRANT and both sweep PUBLIC; two detectors written first and confirmed failing, and the future-table one also asserts TRIGGER and REFERENCES go, because the reset states the PERMITTED set rather than listing dangerous privileges |
| R3S-2 | tenancy | `ConnectionProvisioningGuardMiddleware`'s `doctrine.middleware` tag was **asserted nowhere**: deleting it left the suite green at 3621 assertions against a baseline of 3623 — two fewer, no failure. Round 1's headline P0 shape, aimed at the one control that refuses a connection whose role can bypass row security, own the policed tables or TRUNCATE them — i.e. the control that would have refused the connection R3S-1 armed | P1 | **CLOSED `849a52d`** — asserted on `default` and refused on `owner`, with a mutant |
| R3S-3 · R3C-4 | tenancy, correctness | The `var/cache/test` discard that fixed round 2's headline P0 was itself **enforced by nothing** (neuter it, suite green) and **failed silently** (two `@` suppressions, no post-condition) | P1 | **CLOSED `849a52d`** — a post-condition that refuses to run the suite at all, naming the ownership mismatch, plus a case asserting the compiled container was built during THIS process. Not a grep of `bootstrap.php`: it asks the container file when it was written, so it survives the discard being reimplemented another way |
| R3S-4 | tenancy | **The capability assertions ran nowhere.** `infra/.env` set `TWES_ASSERT_REVOKED_CAPABILITIES=1` since the tier landed; it was absent from the `x-api-environment` anchor, every Dockerfile and the entrypoint, so it was a compose INTERPOLATION variable that never entered a container, and `api/.env`'s `0` won. The two assertions whose own docblock says *"this is the only place they can run at all"* ran in no place at all | P1 | **CLOSED `73d1686`** — and it could not simply be plumbed: `infra/database/init/10-roles.sh` performed NO large-object revocation and all six writers ship with a NULL `proacl`, so wiring the flag alone would have made the stack refuse every connection. Both halves landed, the REVOKE block taken from the checker's own `LARGE_OBJECT_WRITERS` rather than written independently. `pg_largeobject` cannot carry RLS at any privilege level, which makes this the TRUNCATE class by another route |
| R3S-5 · R3C-3 | tenancy, correctness | Round 2's cache-key fix **guarded a route the shipped driver cannot take**. `PDO\PgSQL\Driver::constructPdoDsn()` emits only host, port, dbname and the ssl/gss parameters — never `options` — so `$params['options']` is inert. The route that DOES pin a tenant is `PGOPTIONS` in the process environment, which is not a connection parameter and cannot be keyed on at all. The panel reproduced a pinned process reading another tenant's row inside a transaction the application believed unbound, on a warm entry written by a clean one; the pool is `@cache.app`, cross-process, shared by api, worker and scheduler through one volume | P1 | **CLOSED `73d1686`** — the key stopped being the control. Both per-session assertions now run on EVERY acquisition and only the catalogue ones stay cached, which is what the ~10.8 ms is actually for. The case drives the real vector with `putenv(PGOPTIONS=…)` against a deliberately warmed cache |
| R3S-6 · R3C-1 · R3K-6 | **all three** | **The state-only branch SILENTLY DISCARDED a changed child set and returned success.** `save()` on an already-numbered document issued a state-only `UPDATE` and never touched the lines or charges, so a caller proposing different ones got a 200 and no change. Its other half: that refusal reached neither the port nor the test double, so `InvoiceRepository` promised idempotency its own adapter refused — in the interface whose docblock argues a guarantee must be expressible for a non-Postgres adapter | P1 | **CLOSED `849a52d`** — the branch now COMPARES the child rows and refuses a difference, NUMERICALLY for the decimal columns so an identical re-save is not refused by `NUMERIC(21,6)` returning `1.000000` for `1`. Stated in the port, and the double's list of unreproduced guarantees closed at three with the new one named |
| R3S-7 | tenancy | `$verifiedInThisKernel` sits in front of the pool and is governed by no TTL. The class's docblock reasons about PHP-FPM; `infra/compose.yaml` runs the worker and scheduler with `--time-limit=3600`, so in the one RESIDENT runtime this project deploys the real window was 3600 s against a documented 300 | P2 | **CLOSED `73d1686`** — `ResetInterface`, on the same boundary the tenant context and the session releaser reset on. **That exposed a correction to round 2**: the `kernel.reset` tag never appeared because `autoconfigure: false` strips EVERY autoconfiguration [measured: `--tag=kernel.reset` → 0 hits with the flag on, 2 with it off]. Round 2 called that flag "cosmetic" — true of the connection-scoping question it was measured against, false in general |
| R3C-2 | correctness | The wide `catch (\InvalidArgumentException)` was **still** on the create path — the **third** instance of one defect, after I wrote *"a full-set miss, found by the second MAXIMAL round"* into the second file and did not check the third. It made a hydration failure a 422 naming an internal column, while its own comment said *"every one of these is a caller error"* | P1 | **CLOSED `849a52d`** |
| R3C-5 | correctness | *"ONE test of 949"* was stale within the same four-commit fix series (1016) | P3 | **CLOSED `849a52d`** — direction kept, total removed |
| R3C-6 | correctness | `schema-tenancy.php`'s DSN comment prescribed `array_key_exists` while the code used `'' !== trim($value)` — wrong about the mechanism AND the consequence, since `array_key_exists` is the test that would ACCEPT a declared-but-empty `user=`, the opposite of the rule wanted | P3 | **CLOSED** — corrected in place, with the consequence spelled out (libpq falls back to the OS user, so the gate would accept a DSN carrying no usable credential) |
| R3C-7 | correctness | `save()`'s upsert freely rewrote `type`, while `load()` FILTERS by it — unreachable today, reachable the moment Wave 2's quote repository writes the same table | P3 | **CLOSED** — `AND document.type = EXCLUDED.type` on the upsert predicate |
| R3K-7 | completeness | `DocumentIdentityTest` did not pin the group-length CEILINGS: `{12}` → `{12,}` and `{8}` → `{8,}` both survived. Under the first, a 37-character id reaches `WHERE id = :id` and raises `invalid input syntax for type uuid` — a **500 where that predicate exists to give a 404** | P2 | **CLOSED `849a52d`** — four cases; the first mutant now kills 2, the second 1 |
| R3K-11 | completeness | The **whitespace axis** of the new DSN parse was unpinned — deleting either `trim()` survived all three of round 2's meta-cases | P2 | **CLOSED** — two more cases, in OPPOSITE directions because neither stands in for the other: `trim($key)` is what ACCEPTS ` user =someone` (libpq tolerates it, so refusing is the very trap the relaxation removes), `trim($value)` is what REFUSES `user=   `. Both mutants killed |
| R3K-9 | completeness | `api/composer.json`'s `scripts-descriptions` described `gate:test` as running `e2e` when it EXCLUDES it, and `gate:e2e` had no description at all | P2 | **CLOSED** — both written, the first phrased as an exclusion so a suite added later is included by default |
| R3K-10 | completeness | `make gate ## Run EVERY tier's quality gate` invoked api, infra and make only: the **admin and mobile gates were reachable from no target**, existing solely as prose. The same defect the 2026-08-05 naming ruling closed, one level up | P2 | **CLOSED** — `gate-admin` and `gate-mobile` added and wired into the aggregate, rather than narrowing the help text (which would have re-opened the axis by writing the exception down). Each FAILS rather than skipping without its toolchain, matching `gate:licences` and `gate:schema`; the cost — `make gate` is red on a machine with no Flutter — is stated in the Makefile rather than discovered |
| R3K-12 | completeness | Three stale package counts in `CLAUDE.md` — **four, in fact**, the fourth being the dated citation inside the § Gotchas entry whose own subject is stale counts | P2 | **CLOSED** — every TOTAL removed and the PROPERTY (`0 without source`, `phpstan not in lock`) stated instead, which is what all four sentences were actually asserting and the only part that never moved |
| R3K-13 | completeness | My own R2K-12 closure claimed the sentence *"now carries the derivation rather than a number"* while it carried BOTH | P2 | **CLOSED** — the number is gone. Narrower lesson than "counts drift": **adding a derivation next to a number does not remove the number**, and a closure note is written by the author reading their new text rather than the sentence as it now stands |
| R3K-15 | completeness | `InvoiceResource`'s *"EVERY ANSWER THIS OPERATION CAN GIVE IS DECLARED"* omitted the reachable 500 | P3 | **CLOSED** — narrowed to *"EVERY ANSWER A CALLER CAN ACT ON"*, with the 500's deliberate absence argued from § "Translation keys": our own faults carry their detail in the log, and declaring a 500 per-operation invites the client-side handling that ruling exists to prevent |
| R3K-1 · R3K-2 · R3K-3 · R3K-4 · R3K-5 | completeness | **Five unswept stale claims**, each true when written: this file's opening still listed the HTTP surface, repository and mapper as "what remain"; `bin/console` still described as using the classic bootstrap (refuted in § Gotchas 2026-08-02); the AGREED ruling that API Platform is "NOT installed and scheduled NOWHERE"; a **FIFTH** site of the refuted `autoconfigure: false` claim, in this file's own AGREED ruling — the decision record the claim ORIGINATED in — after § Gotchas said "all three" then "all four"; and `infra/README.md` still saying `bind()` has "zero call sites" | P1 | **ALL CLOSED** — each corrected in place. § Gotchas no longer counts the sites at all: **a correction that ends by counting the places it was applied invites exactly one more place**, and this file has made that mistake three times about one sentence |
| R3K-8 | completeness | `THIRD-PARTY-NOTICES.md` carried **two superseded empty tier sections** — `## TypeScript / Angular (admin)` and `## Dart / Flutter (mobile & desktop)`, each declaring *"No dependencies yet — not scaffolded"* with an empty table — sitting directly above the populated `## Angular admin` and `## Flutter client` sections. The licence gate could not see it: it checks that each direct requirement appears ANYWHERE in the text | P1 | **CLOSED** — both deleted. The one substantive sentence they carried (no `admin-portal` code is used, so its Attribution Assurance License attaches nothing) was MOVED into the populated Flutter section rather than lost, which is the part a careless deletion would have got wrong |
...
1. **~~`discardSessionState()` when a connection is RETURNED~~ — WIRED 2026-08-06 as `SessionStateReleaser`, and
   "RETURNED" needed reinterpreting because THERE IS NO POOL.** This list speaks throughout of "the pool"; PHP-FPM
   gives one process per request and closes the connection with it, so under HTTP there is no return to intercept.
   The moment that DOES exist is Symfony's own — `ServicesResetter` between units of work, which Messenger's
   `ResetServicesListener` invokes after every message [Verified in `vendor/symfony/messenger/`] — and a worker
   consuming jobs for DIFFERENT TENANTS on ONE connection is precisely the hazard described below. So the service
   implements `ResetInterface` and autoconfiguration tags it `kernel.reset` with no manual wiring [Verified:
   `debug:container --tag=kernel.reset`]. Worker mode would add a second, per-request trigger; it is blocked until
   Wave 10, and this service is already what has to run when it lands. Three mutants pin it: swallowing the eviction
   → 1 failure, never calling the cleanup → 3 failures, connecting eagerly to clean an unopened connection →
   1 failure. **Original text follows, because its reasoning is still the reasoning:** A temporary table and a `CURSOR WITH HOLD`
...
configuration and neither specified (round 15). **BOTH LANDED 2026-08-21 as `company_settings`**, and this
paragraph is amended in place rather than left saying the table "must be created" beside the migration that
created it. What shipped is exactly these two columns and nothing else — `Version20260820120000`, keyed
`PRIMARY KEY (company_id)` alone because the table holds one row per tenant, RLS-enabled from
`policySqlFor()` like every other tenant-owned table, and reachable over HTTP as `GET`/`PUT /api/settings`.
Every OTHER value this plan elsewhere calls "company configuration" is still owed and gets its column when the
feature needing it lands; a settings table is not an invitation to add columns in anticipation.
The specification below is what was built: `number_pattern_width` non-null `smallint CHECK (>= 1)` — whose **lower** bound matches
...
`In:` list). ~~**Neither can be built without worked examples from the developer**~~ **— BOTH SUPPLIED 2026-08-23, and this
wave is unblocked.** A line discount REDUCES the VAT base; a document-level discount is allocated PRO-RATA BY BASE;
inclusive tax extracts PER RATE GROUP, reusing `VatRoundingPoint` rather than adding a parameter. The worked numbers
are in § Decisions Log, 2026-08-23 10:30, and the fixtures land in `docs/spec/pricing-vectors.json` with this wave.
**ONE sub-question remains open and it does not block:** which figure absorbs a half-grid TIE when extracting
inclusive tax — moot at Tunisia's rates and live only at France's 20 %, so it may travel to Wave 5. Inventing those
numbers is precisely what this domain exists not to do, which is why they were BLOCKED rather than merely owed — and § "Awaiting the developer" is where both are
...
| R22-6 | **`$ownerConnection` walks past the owner-connection gate.** Symfony's `registerAliasForArgument` resolves a parameter NAME — `Doctrine\DBAL\Connection $ownerConnection` is in this app's compiled container — so a plain constructor parameter gets the table-owning role with none of the six spellings. `#[Target(name: 'owner')]` too, since `Target::$name` is a named argument. An enumerate-the-spellings text gate cannot close a name-based DI mechanism | **P0** | **CLOSED 2026-08-22** — moved to the COMPILED CONTAINER, as this row prescribed. `RefusePrivilegedConnectionAliasesPass` removes every alias resolving to a non-default connection, deriving the ids from the `doctrine.connections` and `doctrine.default_connection` parameters so nothing is written down and a third connection is covered the day it is added. [Verified: `debug:container 'Doctrine\DBAL\Connection $ownerConnection'` → *"No services found"*, the `#[Target]` variant gone from `debug:autowiring`, both default siblings intact, `lint:container` OK and `doctrine:migrations:list` still resolving the connection by registry name.] It runs in the before-optimization stage — which is what precedes autowiring, NOT the priority, a claim the wiring test refuted in its own docblock. **The two routes then fail DIFFERENTLY, measured with a throwaway probe rather than reasoned about, because the first draft of this row and of three other sites claimed both were a compile error:** `#[Target]` naming a privileged connection IS one (*"no such target exists. Did you mean to target `default`?"*), while a parameter merely NAMED after it is not — autowiring falls back to the bare type alias, so it silently receives the DEFAULT, restricted connection [Verified: the probe reported `current_user` as the runtime role, not the owner]. Both are fail-SAFE, which is the property that matters; they are not the same mechanism, and overstating it is how a reader stops checking a tenancy control. Three mutants: dropping the registration reddens all three wiring cases, exempting by NAME instead of by identity reddens the renamed-connection case, and removing aliases inside the resolve loop (the first draft, caught by its own test) leaves a CHAINED alias alive |
...
| R22-8 | **The owner-connection gate never scans `api/config`.** Its spellings include the service id, described as the raw-`get()` route, while the scan is `git ls-files -- api/src`. `arguments: ['@doctrine.dbal.owner_connection']` in YAML hands over the owning role with zero hits. `api/bin/console` and `api/public/` are out of scope too | **P1** | **OPEN — and the stated remedy was WRONG, corrected here in place.** It read *"subsumed by the compiled-container check"*; it is not. R22-6's pass iterates `getAliases()`, and `arguments: ['@doctrine.dbal.owner_connection']` in YAML is a `Reference` on a definition, not an alias — so the pass does not touch it and the scope gap survives R22-6's closure untouched. Still owed: widen the gate's roots to `api/config`, `api/bin` and `api/public` (14 tracked files, `services.yaml` among them), derive the spellings per non-default connection from `doctrine.yaml` instead of the six literals, decide comment handling ONCE (the gate reported R22-6's own docblock as configuration — the shape recorded against `no-forgeable-tenancy-in-production.sh`'s first version), and assert the DEFAULT connection's `url:` names the restricted credential, since `default_connection: owner` or a swapped DSN would silently make every ordinary `Connection` the table owner with the gate green |
...
**RECONCILIATION, 2026-08-23 — `d3cbcb9`'s commit message said *"Wave 1's last open P1 is closed; every row
still open is P2 or P3"*, and that is TRUE ONLY OF THE WAVE 1 BOUNDARY TABLES.** It reads wider than it is, and a
MAXIMAL panel reads this file: an unreconciled row labelled P0-and-OPEN is either a finding to re-file or one to
fabricate, and both cost a round. So, stated once, here:

- **R24-3 is a genuine open P0 — and it is LATENT, not live.** [Verified 2026-08-23 against the migrated `twes_in`:
  `pg_class` holds **no** relation of kind `v`, `m` or `f` in any non-system schema, so there is no view,
  materialized view or foreign table for the gap to leak through today.] What is open is the DETECTOR: a reporting
  view that aggregates `company_id` away carries no tenant column, so both the gate and the behavioural suite
  classify it "not tenant data" and skip it. It becomes live the first time anyone adds a reporting view, which is
  Wave 2 territory, and the plan already records that the fix needs a design decision (whether discovery should
  follow `pg_depend` to a relation's SOURCE) rather than a patch.
- **R22-8, R24-6 and R24-11 are open P1s**, each with its remedy stated on its own row.
- **R28-9 is "CLOSED as a CHECK, OPEN as wiring"**, and the wiring half is explicitly a Wave 10 question.

None of these was introduced by the round-4 closure work, and none of them is in Wave 1's scope. They are named
here so that the claim above cannot be read as covering them.
...
| R26-3 | **The unguessability property has ZERO test coverage.** Mutating `& 0xFFFFFF` to `& 0x0` in the dependency makes identifiers perfectly sequential — literally the `/invoices/1234` enumeration the docblock says a UUID prevents — and all 8 tests pass, exit 0 | **P1** | **CLOSED** — `testTheRandomFieldIsNotMerelyASequentialCounter()`. Mutant: the constant `+1` increment that previously left every case in the file green now fails, naming the counter. **Reopened and closed again twice since**: round 4 showed it pinned the INCREMENT and not the base draw, round 5 showed the cross-process case it gained was satisfied by a `getmypid()` seed |
...
| R27-10 | `HealthController` and `HttpSurfaceTest` asserted *"FrankenPHP runs the app in a persistent worker"* as present fact | **P2** | **CLOSED at round 28, NOT at round 27 — that closure was FALSE.** Neither file was touched: the phrase is LINE-WRAPPED in both (`FrankenPHP runs\n * the app in a persistent worker`), so a single-line replacement matched nothing AND the single-line verification grep returned 0, which I read as success. The fix and the check shared one blind spot. Verified this time with a whitespace-collapsed match |
...
| # | Finding | Sev | Status |
|---|---|---|---|
| R28-1 | **`export APP_RUNTIME=` defeated both halves.** Symfony's Dotenv and compose-go both accept the `export ` prefix; the grep was anchored `^[[:space:]]*APP_RUNTIME=`. One keyword reopened the route the previous commit claimed to close | **P0** | **CLOSED** |
| R28-2 | **`FRANKENPHP_CONFIG` was checked where it can never appear and unchecked where it takes effect.** No compose file declares it; `infra/api/Dockerfile`'s `ENV` does, and the gate named that file in a comment and never read it. The meta-suite cases passed only because the harness injected the key into `compose.yaml` — fixture leakage, inside the fix for a fixture-leakage finding | **P0** | **CLOSED** |
| R28-3 | **The whole control SKIPPED without `docker compose`.** `--dump-rules` had been hoisted above the Docker probe so introspection survived a Docker-less machine; the two checks that need no Docker were left below it. The skip message cites CI; there is no `.github/` | **P1** | **CLOSED** — the text checks moved to a gate that needs no daemon and therefore cannot skip |
| R28-4 | **A `continue` killed the seam check for exactly the case its own message describes.** `if 'APP_RUNTIME' not in env: continue` sat before the seam loop, whose message reads *"a worker can be declared here without APP_RUNTIME changing at all"*. A service with the seam and no `APP_RUNTIME` passed; adding a PERMITTED `APP_RUNTIME` was what made it detectable — inverted in effect | **P1** | **CLOSED**, with a case |
| R28-5 | **`CADDY_GLOBAL_OPTIONS` and `CADDY_EXTRA_CONFIG` were unchecked** — two of the four seams the Caddyfile itself enumerates, and the first splices into the very global block where a `worker` directive lives. A Caddyfile `import` was likewise unread, and `frankenphp { worker … }` on ONE line escaped the line-anchored grep | **P1** | **CLOSED** — all four seams, `import` refused, the token matched anywhere uncommented |
| R28-6 | **The floors ratcheted the WRONG DIRECTION for permission lists.** `permitted_runtime`, `read_only_exempt` and `permitted_cap_add` are permissions, where growth is the danger, and `assert_at_least` guards shrinkage. A reviewer grew them, then stripped `read_only` from the internet-facing `api` service and added `SYS_PTRACE` and `DAC_READ_SEARCH` — memory reading and file-permission bypass — with the suite green. This project's licensing gate keeps three lists and asserts a MAXIMUM on each for precisely this reason | **P1** | **CLOSED** — exact for the runtime, maxima for the permission lists, minima only for refusal lists, and every member pinned by name |
| R28-7 | **The seven worker-mode cases had been weakened to the exit code.** `grep -qiE 'APP_RUNTIME\|worker'` is satisfied by the gate's own unconditional `worker_sources=` counts line, so ANY failure matched. And no case exercised the allow-list on the `.env` routes, so reverting that half to a block-list left the suite green | **P1** | **CLOSED** — each gate's own exact phrases asserted again |
| R28-8 | **`testTwoProcessesAtOneFrozenInstantDisagree()` was satisfied by a `getmypid()` seed** with zero `random_bytes` anywhere, and a reviewer predicted a victim's identifiers from its pid alone with all twelve cases green. Cross-process disagreement is satisfied by a perfect counter, exactly as distinctness and ordering were — the same mistake one level out | **P1** | **CLOSED** — both subprocesses now run in a fresh PID namespace via `unshare --fork --pid --mount-proc`, so each is **pid 1** and a pid-derived seed collides. SKIPS openly where `unshare` is unavailable. **What it still does not cover is stated in the docblock**: a `microtime()`-derived seed would differ even in one namespace, and closing that needs a deterministic clock inside the dependency |
| R28-9 | **The documented operator seam is DEAD WIRING.** All four `CADDY_*`/`FRANKENPHP_CONFIG` variables in `infra/.env` are referenced by no compose file, so none reaches a container — making the Caddyfile's *"the seam by which that is switched on WITHOUT editing this file"* and the Dockerfile's *"every one is overridable from `infra/.env`"* both false | **P1** | **CLOSED as a CHECK, OPEN as wiring** — the new gate reads all four in every scanned file, so the route is covered whether or not it is wired. Whether to wire the seam at all is a Wave 10 question and is recorded as such rather than answered here |
| R28-10 | **R27-10 was recorded CLOSED and neither file was touched.** The phrase is LINE-WRAPPED, so the single-line replacement matched nothing — and the single-line verification grep returned 0, which was read as success. **The fix and the check shared one blind spot**, which is why the closure looked sound | **P1** | **CLOSED** — verified with a whitespace-collapsed match this time. The same wrap hid a sixth operator site, `api/public/index.php` |
| R28-11 | **R27-12's closure was partial and its count was wrong**: one of three sites fixed, *"four sites"* asserted when there were three, and the survivor was this file's own 2026-08-05 RULED entry — with the previous commit inserting two new entries DIRECTLY ABOVE it | **P1** | **CLOSED** |
| R28-12 | **The § Gotchas entry every other document cites still described the deleted block-list** — *"`compose-config.sh` … with a mutant per spelling"*, naming two of the five routes. It is the entry the gates' own failure messages point at, and the register a Wave 10 author reads before deleting the check | **P1** | **CLOSED** |
| R28-13 | `docs/plans/dev-prod-separation.plan.md`'s owed list still presented worker mode as reachable and pending only a certification round — so the most recently completed plan told the next session the only obstacle was a review | **P2** | **CLOSED** |
| R28-14 | `--dump-rules` is a SECOND hand-maintained copy of five rule sets — three copies of the permitted runtime — which the same file's own comment forbids (*"a second hand-maintained list is the drift this repository has been bitten by repeatedly"*). The floors therefore pin the dump, not the applied rule | **P2** | **OPEN** — the honest fix is to derive the dump from the enforcing code, which means restructuring how the bash gate hands its constants to the embedded Python. Recorded rather than patched |
| R28-15 | `.claude/agents/tenancy-security-reviewer.md`'s portal check — *"is the document token unguessable (not a sequential ID, not a short hash)?"* — would PASS a UUIDv7 token, certifying the artefact the ruling forbids. Filed at round 4, still open | **P2** | **OPEN** — an agent definition needs the developer's authorisation; the diff has been presented twice |
| R28-16 | Count inconsistencies: a `# FIVE ROUTES` comment above a seven-item loop; *"Three mutants… the invented one"* counting a value that appears only in comments; *"every member pinned by name"* covering 6 of 13 | **P3** | **CLOSED** |
| R28-17 | Two false positives in the previous text pass: a legal dotenv inline comment, and a CRLF file whose message asserted a value differs from a string it rendered identically to — the `DATE_ATOM` precision defect recurring in the file it was fixed alongside | **P3** | **CLOSED**, and both are now cases in the false-positive direction |

**The lesson, and it is the one this whole sequence keeps teaching at successively higher levels.** Round 27
replaced a block-list of dangerous VALUES with an allow-list, and called it *"the first fix that closes a CLASS
rather than an instance"*. It was — for values. Round 28 then got past it four ways by using LOCATIONS the gate
did not enumerate. The replacement inverts that axis too: `worker-mode-blocked.sh` derives its file set from
`git ls-files`, so a new configuration file is in scope without anybody remembering. **Two axes are now closed by
construction rather than by enumeration; that is not a guarantee there is no third.**

**And the harness keeps producing its own findings.** Round 28's fixes were verified with ten mutants, one of which
first reported SURVIVED — because the mutation script's shell escaping was wrong and it never applied. That is the
...
### Certification round 30 — the transformations became the attack surface

**Twenty-eight findings from a full three-lens panel** (security 7, completeness 12, correctness 9, with overlap).
Frozen commit: **`31eaeba`**. The freeze held this time: no write happened between spawning the panel and the last
lens reporting [Verified: `git status --porcelain` empty throughout].

**The verdict on round 29's inversion: necessary, and it introduced three new P0s of its own.** Scope, knobs and
values were genuinely inverted — the panel confirmed all 84 excluded paths are prose, tests, gates and `.claude/`
with no configuration file among them, that the four ratchets point the right way, and that every previously
existing refusal is load-bearing. But the rewrite moved the enumeration problem from *which values, locations and
paths* to **which text transformations**, and a transformation set is exactly as unenumerable. Three of the eight
P0s live in normalisation code that round 29 added.
...
| R30-22 | P3 | **FIXED** | The tally `14, 24, 18, 26, 31` omits round 28 entirely, though the same diff rewrote that round's header |

**What the panel could not break, recorded so a later round does not re-spend it:** the `APP_RUNTIME` value rule
against every dialect attack tried (`symfony/dotenv`, compose-go, BuildKit); `{env.FOO}`, Caddy's other placeholder
syntax, as a seam route; YAML anchors, aliases and block/folded scalars on a seam; every `EXCLUDED_PATTERNS` member
(all 84 excluded paths enumerated — no `.env`, Dockerfile, Caddyfile, compose file or `api/config/**` among them);
the `import` refusal including nested and globbed forms; the `clean-case-inline` declaration being self-satisfying;
fixture fidelity across every fixture in the suite; and the fast reject as a scope hole (Caddyfiles bypass it, the
`composer.json` check is inside the loop). Every numeric claim round 29 made reproduces except the 20.8s fixture
figure, whose subject no longer exists.

**The lesson, and it is the same one at a new level.** Round 29 replaced "enumerate the forbidden values" with
"enumerate the forbidden locations", then "the forbidden paths", and each was outflanked. Round 29's answer was to
enumerate nothing — but it kept a NORMALISATION step, and R30-2, R30-3 and R30-4 are all attacks on the
normalisation rather than on the rules. **The transformation is an enumeration too.** The structural fix is to stop
transforming per line: join line continuations into LOGICAL lines before the loop, give the seams the same
whole-line allow-list `APP_RUNTIME` has (so there is no value to extract and nothing to collapse), identify
Caddyfiles by what the Dockerfile `COPY`s rather than by filename, and put `SERVER_NAME` under the allow-list
