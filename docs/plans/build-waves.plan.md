...
| R4C-9 | correctness | A comment names `uasort` where the code calls `usort`, and the explicit position tie-break is deletable with both suites green. Judged an **equivalent** mutant (input is already in ascending position order and PHP 8's sorts are stable, exactly as the comment says), so the finding is the wrong function name on a claim about a tax figure | P3 | OPEN |
...
| R4K-7 | completeness | A `[Verified]` mutant citation in `CLAUDE.md` and in the hook suite whose totals no longer reproduce (`28 passed` now, `27` claimed) — in the file whose own § "Claude config" says of that suite *"it reports its own case count; none is written here"*. Mechanism intact, mutant still kills; only the numbers are stale. The reviewer first mis-applied the mutant and briefly read the survival as a P1, which is worth keeping | P3 | OPEN |
| R4K-8 | completeness | The § Gotchas 2026-08-06 table partitioning gates by `git ls-files` mode names two gates in the `--cached --others` row; there are three. Same hand-written-enumeration cause as R4K-4, one level up: the third gate landed 2026-08-05, the table was written 2026-08-06 | P3 | OPEN |
| R4K-9 | completeness | The functional-suite description partitions "it" into two named groups covering four of six classes — and the omitted one is `TenantBindingWiringTest`, the one § Gotchas 2026-08-07 calls indispensable (*"Both, or neither is enough"*). The R2K-12 shape recurring on the sentence that replaced it | P3 | OPEN |
| R4K-10 | completeness | `CLAUDE.md` and `scripts/claude-bootstrap/CLAUDE-global.md` disagree about `.claude/settings.json`: one says `deny: []` AND `ask: []`, the other verifies there is no `ask` key. No behavioural difference (absent ≡ empty), but the intent sentence reads as describing a key that is there | P3 | OPEN |
