...
| R4K-4 | completeness | `no-forgeable-tenancy-in-production.sh` — a SECURITY gate in `gate:architecture` — is routed by the write-time hook for no file type AND is absent from that hook's "deliberately not here" list, so no decision is recorded either way. It fell through because the hook suite's coverage check is **one-directional**: it asserts only that the three excluded gates stay out, never that a non-excluded gate is in | P2 | OPEN |
| R4K-5 | completeness | `CLAUDE.md`'s *"every gate answers `--dump-rules`"* is false for two of fourteen, and both treat the flag as a normal run rather than refusing it, so a consumer cannot distinguish "no rules" from "ran the whole gate". `test-gates.sh` reads the flag from eight gates BY NAME and never asserts the universal — the hand-picked-versus-derived split that row is itself arguing against | P2 | OPEN |
| R4K-6 | completeness | Three authored `.xlf` catalogues carry no SPDX identifier (invariant 8(c)), `api/translations` is under no search root, `xlf` is in no documented exclusion — and the only trace of a decision is a `grep -v '\.xlf$'` proven to be **dead code** (replacing it with `cat` gives a byte-identical result). Every other absence from `EXTENSIONS` has a reason recorded beside it | P2 | OPEN |
