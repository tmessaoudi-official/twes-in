...
document lifecycle, numbering and the `Invoice` aggregate, all under `api/src/Domain/Document/`. **Wave 1's persistence, migrations and schema gate all LANDED on 2026-08-01, and the three this sentence then listed as remaining — the HTTP surface, the repository and the mapper — have ALL landed too**: the repository and mapper on 2026-08-06, the READ path with them, and the WRITE path on 2026-08-07. **What Wave 1 still owes is a DIFFERENT list**: `Client`, contacts, `Product` and the tenant settings table, none of which is an invoice-core deliverable. This sentence named the wrong three for two commits after the invoice core closed out, which is the same shape as the sentence it itself corrects, one wave later. This sentence said they were "blocked on Composer" until round 21 found it — two commits after the passage below was corrected, which is the correction-appended-below-a-false-statement shape `CLAUDE.md` § Gotchas records four times, committed by me twice more while fixing other instances of it. Amended at round 13, which found this sentence and the one in Wave 0's remainder section still saying Wave 1 had not started — in a file the landing commit itself edited. Read
