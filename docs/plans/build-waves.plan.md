...
**What is true on 2026-08-22:** the client/contact half is refuted — `Domain/Client/` holds four types, and
`client` and `client_contact` are real policed tables. The Product half still stands exactly as written. What
replaced the retracted claim is NOT "the wave is complete" but a narrower and still-open list: the
`/api/clients` HTTP surface, the `document` → `client` link, and Product. The tenant SETTINGS table, which this
