...
classes (**SIX as of 2026-08-07, and this said FIVE two commits after `TenantBindingWiringTest` landed** — round 2's
R2K-12, and the third time this one sentence has gone stale. Derive it: `git ls-files api/tests/Functional | grep -c
'\.php$'`. A count written beside the thing it counts, which is the defect this file
...
| R2K-11 · R2K-12 | completeness | Two stale counts: "two rounding constants" (one), and "FIVE functional classes" (six) | P3 | **BOTH CLOSED** — the second now carries the derivation (`git ls-files api/tests/Functional \| grep -c '\.php$'`) rather than a number, because it had gone stale three times |
