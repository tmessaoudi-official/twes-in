...
## Wave 1 — Client & the invoice core — **THE INVOICE CORE LANDED; THE WAVE IS *NOT* COMPLETE** (domain 2026-07-31; schema 2026-08-01; repository, savepoint guard, boundary rule and the connection lifecycle 2026-08-06; the invoice HTTP surface 2026-08-06 read / 2026-08-07 write; the `e2e` suite 2026-08-07; the tenant settings table 2026-08-21). **Still owed: the `document` → `client` link, and NOTHING ELSE** — Client (+ contacts) and Product both LANDED on 2026-08-22 (domain, schema, persistence and a `POST`/`GET` surface each; `PUT`/`DELETE`/collection-`GET` are deliberately absent from both, with their reasons on `ClientResource` and `ProductResource`), and `document` still carries no `client_id`, which is the one thing between this wave and complete. Client was recorded as PART-LANDED between its three commits rather than struck whole, and the tenant settings table LANDED on 2026-08-21 (`company_settings`, `CompanySettings`, its port and Doctrine adapter, retiring both hardwires), so it is struck from this list in place rather than left standing beside its own delivery. See the retraction below, and note that the heading said `WAVE COMPLETE` for two commits AFTER that retraction was written 45 lines beneath it, which is this file's own append-don't-replace defect landing on the one line a reader and every table of contents render first.
...
| R4K-2 | completeness | `infra/README.md` calls `/api/currencies` *"the one implemented resource"*; there are two resources and five operations | P2 | OPEN |
