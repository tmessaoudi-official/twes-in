...
| R22-18 | The null-`polqual` tolerance certifies an UNREADABLE table as "canonically policed": `FOR ALL` omitting `USING` does not reuse `WITH CHECK`, and `FOR INSERT` is already refused by the `polcmd` check, so the arm admits only this. Fails closed | **P2** | **CLOSED** `this commit` — the tolerance is now asymmetric: a NULL `WITH CHECK` is accepted (`FOR ALL` reuses `USING`), a NULL `USING` is refused (nothing reuses `WITH CHECK` for reads). Both directions asserted and mutation-tested |
