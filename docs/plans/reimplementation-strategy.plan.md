...
- [2026-07-29 22:40] AGREED: the permissive set for anything **distributed** is exactly nine identifiers — MIT, Apache-2.0, BSD-2-Clause, BSD-3-Clause, ISC, **0BSD, MIT-0, CC0-1.0, BlueOak-1.0.0** — with **CC-BY-4.0/3.0 tolerated for DEV-ONLY build-time data** and nothing else. `CLAUDE.md` 8(a), `LICENSING.md`, the reviewer charter and `scripts/gates/dependency-licences.php` now all state the same list; round 6 found the gate enforcing more than the invariant allowed, because I widened the gate without amending the invariant — a licensing decision taken as a build fix, which invariant 10 forbids. Only ONE runtime dependency needs any addition: `tslib` (0BSD).
