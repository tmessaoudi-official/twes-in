...
final readonly class ChangeSettingProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): SettingResource
    {
        ...
        $settingContext = SettingLevel::Role === $level ? $this->access->contextOfRole($company, $data->roleId) : $this->access->contextOf($company);
        ...
        return SettingResource::of($setting, $this->access->writableLevels($setting->definition, $this->access->mayShare($company)));
    }
}
