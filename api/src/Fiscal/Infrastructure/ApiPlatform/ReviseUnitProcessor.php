...
final readonly class ReviseUnitProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): UnitResource
    {
        ...
        try {
            $unit = $this->manage->revise(
                $company,
                CompanyPath::identifier($uriVariables, 'unitId'),
                new UnitChanges($data->name, $data->decimals, $data->isActive, $data->sortOrder),
                $this->guard->account()->getId(),
            );
        } catch (UnitNotFound $missing) {
