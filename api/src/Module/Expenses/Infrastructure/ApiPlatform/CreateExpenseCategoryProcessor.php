...
final readonly class CreateExpenseCategoryProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): ExpenseCategoryResource
    {
        ...
        try {
            $category = $this->manage->create($company, $data->name, null === $data->parentId ? null : Uuid::fromString($data->parentId), $data->isActive, $this->guard->account()->getId());
