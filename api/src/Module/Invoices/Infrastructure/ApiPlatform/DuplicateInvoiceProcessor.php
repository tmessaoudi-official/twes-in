...
/**
 * Copies a document into a new draft; a credit note answers 409, since it belongs to the invoice it corrects.
 *
 * @implements ProcessorInterface<mixed, InvoiceResource>
 */
final readonly class DuplicateInvoiceProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): InvoiceResource
    {
        ...
        try {
            $copy = $this->manage->duplicate($company, CompanyPath::identifier($uriVariables, 'invoiceId'), $this->guard->account()->getId());
        ...
        return InvoiceResource::of($copy, $this->totals->figures($copy));
    }
}
