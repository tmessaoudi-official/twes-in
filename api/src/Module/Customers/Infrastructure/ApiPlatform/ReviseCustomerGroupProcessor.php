...
final readonly class ReviseCustomerGroupProcessor implements ProcessorInterface
{
    ...
    public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): CustomerGroupResource
    {
        ...
        try {
            $group = $this->manage->revise($company, CompanyPath::identifier($uriVariables, 'groupId'), $data->name, $data->description, $this->guard->account()->getId());
        ...
        return CustomerGroupResource::of($group, $this->manage->customerCount($group));
    }
}
