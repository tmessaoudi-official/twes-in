...
/**
 * **THE ONE DEFINITION OF AN IDENTIFIER, AND THE THREE PROPERTIES ITS DOCBLOCK CALLS LOAD-BEARING.**
 *
 * These cases arrived with `DocumentIdentity::isWellFormedId()` and MOVED here with the rule itself, unchanged.
 * They are kept verbatim rather than rewritten because they are the record of a real failure: the predicate was
 * extracted once before with **no test of its own**, two of round 2's lenses found that independently, and each
 * measured the same thing — every specific rule the docblock names could be deleted with the entire suite green.
 *
 * ```
 ...
 * ```
 *
 * Only the coarsest mutant died, and it died in a *functional* test about HTTP 404s. So the first extraction
 * moved a correctness rule into `Domain/` — trivially unit-testable, zero collaborators — and left its only
 * coverage behind a PostgreSQL dependency, with the two shapes the docblock names as historical defects untested
 * in either place.
 *
 * **Each case below corresponds to one of those three properties, and each is a real defect rather than a
 * hypothetical one:**
 *
 * - `/D` — without it PCRE's `$` also matches BEFORE a final newline, so `"…e1f0\n"` is accepted. Two unequal
 *   strings for one id, and this value is used as a key.
 * - the anchors — without `^` an unanchored pattern accepts an id with a payload PREPENDED, and this value
 *   reaches a `WHERE` clause.
 * - case sensitivity — uppercase and the braced/urn forms are refused rather than normalised, because two
 *   spellings of one id compare unequal as strings.
 */
...
final class IdentifierTest extends TestCase
{
    ...
    {
        self::assertTrue(Identifier::isWellFormed(self::CANONICAL));
    }
    ...
    {
        self::assertFalse(
            Identifier::isWellFormed($id),
            var_export($id, true) . ' must not be accepted as an identifier',
        );
    }
}
