...
| **A non-superuser application role, without `BYPASSRLS`** | Row-level security does not apply to a superuser. `assertConnectionCannotBypassPolicies()` exists to fail loudly if this is got wrong. **It must run WHEN A CONNECTION IS ACQUIRED, and in CI — not once at boot**, which this row said for a round while the method's own docblock said the opposite: every question it asks is answered point-in-time, and a privileged role can change the answer under a connection that has already been certified, so "at boot" understates the exposure by the whole life of the process. **IT IS WIRED SINCE 2026-08-06** — `ConnectionProvisioningGuardMiddleware`, a DBAL driver middleware on the `default` connection, runs it at connection ACQUISITION and caches the verdict per (role, database) against a measured ~10.8 ms of catalogue queries. This row said it had **zero call sites** and named the wiring as owed, which was true when written and false from that commit. `scripts/gates/schema-tenancy.php` additionally refuses a runtime role that is `rolsuper`, `rolbypassrls` or `rolreplication`, which covers CI; the middleware is what covers the running process, and the two are deliberately both. |
