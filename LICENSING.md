...
**2. Every dependency must be PERMISSIVE — not merely AGPL-compatible.** `THIRD-PARTY-NOTICES.md` is
...
is not good enough here. When a needed library is copyleft-only, STOP and ask (`CLAUDE.md` invariant 6);
there is no silent remedy, and the notices file records what is present rather than what to do.
