...
| `no-forgeable-tenancy-in-production.sh` | **`TWES_TRUST_TENANT_HEADER` is `0` in every tracked file.** `HeaderTenantResolver` trusts an `X-Tenant-Id` header verified by nothing, so enabling it on a reachable deployment lets any caller act as any tenant whose id they can produce — a cross-tenant read of every client's invoices. It exists only so Wave 1's HTTP surface could be exercised before Wave 7 writes authentication, and it is one of **several independent controls** — the resolver refuses loudly when not permitted, both committed dotenv files set `0`, this gate refuses any other value in a tracked file, and `compose-config.sh` refuses one in the RENDERED configuration. **No count is written here**, because the one that was said "three" while the third was inert: `infra/.env`'s value was referenced by no compose file, Dockerfile or entrypoint, so it never entered a container and `api/.env`'s `0` decided by accident — three of three was two of three, for the deployment the docblock itself calls riskiest (round 4, R4S-1). Wired 2026-08-23 as a LITERAL in the `x-api-environment` anchor rather than `${TWES_TRUST_TENANT_HEADER:-0}`, which THIS gate refuses [Verified: `violations=1`] and which would have made editing `infra/.env` enough to switch the header on. Enumerate the controls by grepping the knob's name. All three axes are INVERTED, from `worker-mode-blocked.sh`'s three defeats: scope is EXCLUSION-based over every tracked file, and an assignment must carry the single permitted value `0` rather than merely avoid `1` — which closes `true`, `on`, `yes`, `"1"` and a value assembled from two files at once. "Assignment" is itself DERIVED (only closing quotes, brackets and whitespace may sit between the knob and an `=`/`:`) rather than a literal-line comparison, because prose legitimately names the knob — and the gate's FIRST version reported **its own subject's PHP docblock** as configuration. Comment leaders are stripped ONCE into a variable every check reads, and there are THREE of them (`#`, `//`, a docblock `*`): the second and third were added because the meta-suite caught a `//` comment whose knob was followed by a colon. Needs nothing installed, so it can never skip. **Deleted in Wave 7** with the resolver, the variable and its cases — not before, or an authenticated resolver would ship beside a forgeable path to the same privilege |
...
| `no-owner-connection-in-application.php` | no code under `api/src/` names the **owner** DBAL connection. `doctrine.yaml` calls the default/owner split a security boundary; round 21 showed it was not one — `debug:autowiring` offers `#[Target('owner')]`, so one line of ordinary application code (the classic "fix the permission error" edit) yields the role that OWNS the tenant tables and can `DROP POLICY`, which `FORCE` does not prevent. A reviewer booted the kernel and disabled row security on `document` in one statement. Originally chosen OVER stripping the autowiring alias, on the ground that stripping closes the attribute and leaves `$doctrine->getConnection('owner')` open — the connection must stay in the registry for the migrations bundle to resolve it. **That ruling was reversed on 2026-08-22 (R22-6) and this cell is amended in place rather than left contradicting what shipped:** DoctrineBundle ALSO registers an alias keyed on the connection's NAME, so a plain constructor parameter reached the privileged role with none of these spellings, and no text gate can close a name-based DI mechanism. `RefusePrivilegedConnectionAliasesPass` (registered by `Kernel::build()`) now strips those aliases, and the two are **complementary halves of one control**, not rivals: the pass owns everything resolved through an alias, this gate owns `getConnection('owner')` and the raw service-id string, which no container pass can see. `PERMITTED_PATHS` is deliberately EMPTY: nothing in `src/` needs it. **Still owed here, R22-8:** the scan is `api/src` only, so `api/config`, `api/bin` and `api/public` are unchecked — a YAML `arguments: ['@doctrine.dbal.owner_connection']` is a Reference rather than an alias, so the pass does not cover it either |
...
bash  scripts/gates/compose-config.sh   # the RENDERED half needs `docker compose` and skips without it; its
                                        #   large-object cross-check runs first and never skips
