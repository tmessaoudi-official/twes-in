...
- `scripts/gates/` — `dependency-licences.php` (four lists, each a maximum, `--dump-rules`),
  `spdx-headers.sh`, `executable-bits.sh`, `icon-buttons-named.sh` (every Material icon button named through `appLabel`) and `outcomes-as-toasts.sh` (what was just done is a toast through `Feedback`; a `role="status"` line only for a page state it names); `tests/` beside them is each gate's own test, run by CI
  before the gate.
