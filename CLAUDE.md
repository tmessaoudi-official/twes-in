...
| `schema-tenancy.php` | the **two properties of a migrated schema that no ATTACK can observe**, plus the DISCOVERY the behavioural suite consumes. **Deliberately made smaller on 2026-08-01** (developer ruling): it used to assert nine properties by reading catalogue metadata, and round 22 produced SIX P0s in those checks while the schema they guard survived every attack both security lenses could build — every confirmed breach was in the checker, none in the thing checked. What remains: **(1)** the tenant column is `NOT NULL`, because under the canonical predicate a NULL tenant matches NOBODY, so the row is invisible to every tenant including the one that wrote it — the one failure mode no cross-tenant read reveals, since an attacker hunting another tenant's rows and the legitimate owner looking for its own both see nothing; and **(2)** the runtime role does not **own** any relation, which a probe cannot see because the dangerous thing has not happened yet — `FORCE` stops an owner *skipping* policies, not *removing* them, so `ALTER TABLE … DISABLE ROW LEVEL SECURITY` is one statement away. That second one is deliberately unscoped to tenant-owned relations: a non-tenant table owned by the runtime role leaks nothing itself, but it proves migrations run as that role, so the NEXT tenant-owned table they create is owned by it too. Not hypothetical — `doctrine_migration_versions` in the local dev database was exactly this on 2026-08-01. Plus **discovery**, which is the input `BehaviouralIsolationTest` attacks, and a **refusal of any relation it cannot classify**, because an unclassified relation goes UNATTACKED by both checks at once — `customer_id`, `client_id` and `account_id` are deliberately NOT treated as tenant columns, since each names a row's *subject* rather than its owner. Its subject set is **every non-system schema** and five relkinds, both narrowings having been reproduced leaks; the `NOT NULL` assertion alone is scoped to `('r','p')`, because `attnotnull` is false on every view and materialized view whatever the base table says. **Isolation itself is no longer asserted here** — see the row below. Its clean-fixture and violation cases live in `api/tests/Integration/Tenancy/SchemaTenancyGateTest.php` (the database is there, not in the meta-suite) and `test-gates.sh` verifies that redirect points at a test that really exists |
| `BehaviouralIsolationTest` (a test, not a gate — named here because it now carries what the row above gave up) | tenant isolation **proven by attack**: two tenants with real rows, and eight attacker goals attempted as the restricted runtime role against **every relation discovery finds** — read, write into another tenant, modify, re-parent, delete, `TRUNCATE`, probe by uniqueness collision, and reference another tenant's row — plus `FORCE` attacked as the owning role, `SET ROLE` escalation into every reachable role, and the unbound-session read. The idea that retires the key-shape axes: to prove no unique key omits the tenant, insert tenant A's row **verbatim** under tenant B. One attack covers PRIMARY KEY, UNIQUE, unique INDEX, an index whose tenant column is merely `INCLUDE`d and an `EXCLUDE` constraint — naming none of them, and covering whatever PostgreSQL 19 adds. Every goal is proven load-bearing by its own mutant, and five cases build a deliberately defective relation and require the attack to report it, so the deletion above is not an act of faith. Anti-vacuity is asserted, not assumed: every discovered relation attacked (compared by NAME), both tenants holding rows, at least one attack proven REFUSED, and a POSITIVE control per relation — without which a relation that is merely empty or unreadable satisfies every cross-tenant assertion trivially |
...
| **Schema tenancy** | `scripts/gates/schema-tenancy.php` — the two properties no attack can observe: the tenant column is `NOT NULL`, and the runtime role owns no relation. Plus discovery, where a relation it cannot classify is a **failure** rather than a skip, because the behavioural suite attacks exactly what discovery reports. **Isolation itself moved to `BehaviouralIsolationTest`** on 2026-08-01 — see § "Architecture" for why, and do not re-add a catalogue check here believing its absence was an oversight | **Runs**. Needs `TWES_SCHEMA_DSN` + `TWES_SCHEMA_USER`, or falls back to the integration suite's `TWES_TEST_DSN` / `TWES_TEST_DB_SUPERUSER` pair. Its clean and violation cases are in `api/tests/Integration/Tenancy/SchemaTenancyGateTest.php` because the meta-suite has no database |
| **Behavioural tenant isolation** | `api/tests/Integration/Tenancy/BehaviouralIsolationTest.php` — eight attacker goals against every discovered relation, each proven load-bearing by a mutant. Part of the `integration` suite rather than `scripts/gates/`, because it needs the full role topology `provision-test-database.sh` builds and not just a superuser DSN | **Runs** (landed 2026-08-01) |
