...
- `api/src/<Context>/{Domain,Application,Infrastructure}/` — `Identity`, `Tenancy`, `Audit`, `Inbox` (the notification
  ...
  `Scanning` (a phone lent to a computer tab as a scanner: `scan_pairing`, the tab's endpoints under its company and
...
- `web/src/app/api/` — TypeScript types generated from the API's OpenAPI document (`make api-types`, gitignored;
  ...
  `appLabel` directive, one string for a control's accessible name and its tooltip; `testing/`: providers specs share (`provideQuietFeedback` records toasts, `effectToasts` reads the kinds said; `announceSaved` plays a live change); `feedback/`: the `Feedback` port (toasts; `effect(key, params, kind)` after a consequential action), `RequestActivity` and its interceptor, the activity bar; `actions/`: `ScreenAction`, whose confirmation must name its `ActionKind` (annulable, corrigeable, définitif), and `kindAmong`, which the toast after an action reads, and `PlannedActions`, what a screen will offer once a planned module ships, drawn « Bientôt » first in either bar; `legal/`: `LEGAL_PAGES`, `LegalFooter` (the copyright and legal-links line closing every page), `CookieNotice` and `STORED_ITEMS` (what is stored, which the Cookies page lists); `tour/`: `TOUR_ANCHORS`, the `data-tour` places a guided tour may point at; `health/`: the API health client; `scan/`: `ScanWedge`, which tells a scanner's burst from typing (the shell opens `ProductScanCard` on one outside a field), the `ScanBus` every scan goes through, `CameraView`, `PhonePairing` (the tab's side of a lent phone) and `ScanOffers` (what the card offers that phone); `qr/`: `QrCode`, a QR code drawn as SVG; `ui/`: the bars, `WINDOW_CLASS`, and a menu's foldable sections (`sectionFolds`, kept in `presentation.folded-sections`) and its edge fade and current entry in view (`NavScroller`); `customer-view/`: `CustomerView`, the per-tab switch that hides on screen what the company keeps from a customer (`hides('cost' | 'supplier-codes')`); `customer-display/`: `CustomerDisplay`, what a sale's scans show the customer display window (`/customer-display`, the `customer-display` feature directory) over a BroadcastChannel; `realtime/`: the one Centrifugo connection's connector, the `X-Tab` interceptor naming this tab, and `LiveChanges` (a page calls `reloadOn(kinds, reload, destroyRef)` to read its data again, quietly, when another tab or member changes those kinds); `settings/`: the `SettingsFacade` port, its API adapter
