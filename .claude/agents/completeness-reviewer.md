...
10. **TENANCY WIRING THAT WAVE 1 OWES — check this whenever a repository or a Doctrine mapping appears.**
   `PostgresRowLevelSecurityIsolation` carries guards that Wave 1's connection lifecycle owes calls to, and **ZERO** of them are on the `TenantIsolationStrategy` port, which declares `bind()` and nothing else. **No count is written here** — round 13 corrected this line from "only one", round 14 found the correction itself carrying three wrong numbers (it said FOUR then enumerated six, said "three are `static`" when four are, and kept a leftover "the other three" from the superseded framing), so the lesson is the one CLAUDE.md states generally: a count in prose beside the thing it counts is the first thing to drift. Derive it: `grep -n 'public static function \(assert\|discard\)\|public function assert' api/src/Infrastructure/Tenancy/PostgresRowLevelSecurityIsolation.php`. As of round 14 the set is `assertStillBoundTo()` and `assertConnectionCannotBypassPolicies()` (instance methods), plus `discardSessionState()`, `assertNoLargeObjectIsReachable()`, `assertConnectionCannotCreateTemporaryObjects()` and `assertConnectionCannotCreateLargeObjects()` (static, so they cannot go on an interface as written) — and `discardSessionState()` now throws `ConnectionMustBeEvicted`, which the pool must act on by CLOSING the connection rather than returning it. The zero-large-objects RULE is a rule plus a capability revocation (`infra/README.md` § "No large objects, ever"), not only a method call. A repository injected with the port reaches none of them without an `instanceof`. `assertStillBoundTo()` guards against a savepoint rollback silently
