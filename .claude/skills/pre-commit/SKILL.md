...
Produce the completion gate table from CLAUDE.md Rule 6 — the reasoning framework installed by `scripts/claude-bootstrap/install.sh`, whose source of truth in this repo is `scripts/claude-bootstrap/CLAUDE-global.md` § 6 (the project-level CLAUDE.md does not exist yet):
